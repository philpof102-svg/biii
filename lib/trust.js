...
function repVertex(rep, minScore = 40) {
  /* ═══ « PAS INTERROGE » N'EST PAS « INTERROGE, RIEN TROUVE » ═══
   * Ce sommet etait le dernier des trois a confondre les deux. Standing (2026-07-27) et settlement
   * (2026-07-27) ont ete separes; la reputation rendait encore `unknown` dans les deux cas, et `unknown`
   * n'appartient pas a l'ensemble UNREAD — donc oracle injoignable => triangle declare COMPLET, sans que
   * rien dans la sortie ne dise qu'un sommet n'avait pas ete lu.
   *
   * L'information existait pourtant: fetchOracle rend {error:'oracle unreachable: …'} quand il ne joint
   * pas, et le handler l'aplatissait sur null une ligne plus bas. La couche du dessous etait prudente,
   * celle du dessus l'effacait — la faute la plus discrete de la famille.
   *
   * `unqueried`: rien ne nous a ete fourni pour ce sommet (ou le producteur dit explicitement qu'il n'a
   * pas pu lire). `unknown`: on a bien recu une reponse, elle ne porte aucun signal exploitable. */
  if (rep && rep.unqueried === true) {
    return { status: 'unqueried',
      reason: rep.reason || 'the reputation oracle was not read — a missing vertex on OUR side, not a '
        + 'statement about the counterparty' };
  }
  if (!rep) {
    return { status: 'unqueried',
      reason: 'no reputation input was supplied for this vertex — unread, which is NOT the same as '
        + 'consulted-and-empty' };
  }
  if (rep.decision == null && rep.verdict == null && rep.score == null) return { status: 'unknown', reason: 'no reputation data' };
