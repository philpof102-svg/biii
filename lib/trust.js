 ...
 *
 * @param st { paidMicro } | null | { unqueried: true, reason }
 */
function standingVertex(st) {
  ...
  if (micro == null || BigInt(String(micro || '0')) <= 0n) {
    return { status: 'none', reason: 'consulted: no proven paid history with this counterparty yet' };
  }
...
function assessTriangle({ reputation, standing, settlement } = {}, opts = {}) {
  ...
  /* COMBIEN DE SOMMETS ONT REELLEMENT ETE LUS.
   * Un triangle calcule a deux sommets n'est pas un triangle, et le lecteur doit le savoir sans avoir a
   * inspecter chaque `status`. `unqueried` est le seul etat qui compte comme non-lu: 'none', 'weak' et
   * 'pending' sont des REPONSES — on a regarde et voici le resultat. */
  const unqueried = Object.entries(v).filter(([, x]) => x.status === 'unqueried').map(([k]) => k);
  ...
  return {
     ...
     * cesse d'etre lu. */
    ...(complete ? {} : {
      incompleteNote: 'INCOMPLETE: ' + unqueried.length + ' of 3 vertices could not be read ('
        + unqueried.join(', ') + '). A missing vertex is not a negative one: this verdict was composed '
        + 'from fewer signals than it appears to have, and a re-run with them configured may change it.',
    }),
  };
}
