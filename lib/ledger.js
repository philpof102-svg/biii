...
function windowRows(rows, window) {
  ...
  const rawFrom = Number((window && window.fromBlockTime) || 0);
  const rawTo = Number(window && window.toBlockTime);
  const from = Number.isFinite(rawFrom) && rawFrom > 0 ? rawFrom : 0;
  const to = Number.isFinite(rawTo) ? rawTo : Infinity;
...
module.exports = { renderReceipt, appendReceipt, summary, reverify, reverifyRoll, renderRoll, brandLine, receiptNo,
  windowRows, blockTimeOf, undatedNote };
