...
function assessRugFields(r, sim) {
  ...
  // and then REMOVED the same evening when it was replayed against those eight. It changed the verdict on
  // none of them and escalated three still-living tokens: zero measured benefit, measured cost. The reason
  // is visible only in the data — seven of the eight rugs carried exactly ONE live flag, the withdrawable
  // pool, and nothing else, so requiring a second flag demanded precisely what the rugs lacked. The rule was
  // written after looking at the failures it was meant to fix, and the extra condition came from intuition
  // rather than evidence. Left documented rather than deleted: the next person to notice that unlocked LP
  // is the real rug mechanism will be right about the mechanism and should not re-derive this same wrong
  // encoding of it. Escalating on that single flag alone is the untested option; it needs a false-alarm
  // measurement on tokens that survive, not another good argument.
  let verdict;
