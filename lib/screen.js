 ...
 */
function verifyFloor(artifact, { expectedFingerprint = null } = {}) {
  ...
  if (!claimed) return { valid: false, computed, claimed: null, reason: 'artifact carries no fingerprint — cannot verify integrity; refusing to adopt.' };
  if (computed !== claimed) return { valid: false, computed, claimed, reason: 'content does not hash to its claimed fingerprint (tampered/corrupted) — refusing to adopt.' };
  if (expectedFingerprint && expectedFingerprint !== claimed) return { valid: false, computed, claimed, reason: 'fingerprint does not match the expected/pinned value — refusing to adopt.' };
  const s = loadScreen(a);
  return { valid: true, computed, claimed, count: s.count, asOf: s.asOf || null,
    reason: 'content matches its claimed fingerprint' + (expectedFingerprint ? ' and the expected value' : '') + ' — safe to adopt. (You never trusted the source; you verified the hash.)' };
}
