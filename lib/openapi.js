 ...
 */
/* Le VERDICT rendu, en JSON Schema — partage par les trois routes puisqu'elles
 * rendent toutes la meme forme (c'est deja `verdictSchema` cote OpenAPI). */
const SCHEMA_SORTIE = {
  type: 'object',
  properties: {
    status: { type: 'string', enum: ['genuine', 'impersonation', 'unsafe', 'unknown', 'blocked', 'clear'] },
...
/* `schema` decrit l'ENVELOPPE `info`, pas le corps de requete — piege qui m'a
 * coute un aller-retour. Le validateur officiel applique ce schema a
 * `info.input` et `info.output` TELS QUELS, donc a `{type, method, bodyType,
 * body}` et `{type, example}` ; le schema du corps se niche dans
 * `input.properties.body`. Se tromper d'un niveau donnait
 * « /input: must NOT have additional properties » quatre fois — une par champ
 * de l'enveloppe — et `extractDiscoveryInfo` rendait NULL.
 *
 * AUCUN `$ref` ni `$id` : la spec rejette toute reference qui ne soit pas un
 * pointeur interne `#/...`, pour empecher SSRF/LFI (CWE-918). Le plus sur est
 * de n'en produire aucun, quitte a repeter la forme du verdict. */
const envelopper = (corps, requis) => ({
  ...
  properties: {
    input: {
      ...
      properties: {
        ...
        body: { type: 'object', properties: corps, required: requis },
      },
    ...
    output: {
      type: 'object',
      properties: { type: { type: 'string', enum: ['json'] }, example: SCHEMA_SORTIE },
      required: ['type'],
    },
  },
  required: ['input'],
...
const BAZAAR_ROUTES = {
  '/x402/vet-address': {
    ...
    schema: envelopper({ address: ADRESSE, resourceUrl: { type: 'string' } }, ['address']),
  },
  '/x402/vet-asset': {
    ...
    schema: envelopper({ address: ADRESSE, claimedIssuer: { type: 'string' }, claimedSymbol: { type: 'string' } }, ['address']),
  },
  '/x402/vet-meme': {
    ...
    /* `chainId` accepte le slug DexScreener ('base') OU un id EVM numerique —
     * la meme tolerance que le schema OpenAPI, sinon les deux documents que lit
     * le meme agent se contrediraient. */
    schema: envelopper({ symbol: { type: 'string' }, chainId: { type: ['string', 'number'] }, address: { type: 'string' } }, ['symbol']),
  },
};
  ...
  const declaration = {
    ...
    extensions: {
      bazaar: {
        ...
        /* SANS ce champ, le validateur officiel rend
         * « Schema validation failed: schema must be object or boolean » et
         * `extractDiscoveryInfo` rend NULL — le facilitateur n'extrait rien.
         * Mesure du 2026-08-15 : la premiere version de cette declaration
         * omettait `schema` et passait pourtant `validateDiscoveryExtensionSpec`.
         * Un reglement aurait donc REUSSI, l'argent serait parti, et le
         * catalogue serait reste vide — exactement ce que la spec annonce :
         * « non-conforming content may be dropped even when verify and settle
         * succeed ». Valider avant de depenser, pas apres. */
        schema: spec.schema,
      },
    },
  };
