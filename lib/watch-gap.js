 ...
 *
 * ⚠️ `Date.parse(0)` NE VAUT PAS 0 — IL VAUT L'AN 2000. `Date.parse` coerce son argument en CHAINE, donc
 * le `0` du repli devenait `"0"`, qui parse comme une annee. Le `|| 0` ne rattrapait rien puisque le
 * resultat est truthy (il fonctionne tres bien pour `"garbage"`, qui donne NaN — d'ou l'illusion que le
 * repli marchait).
 *
 * Mesure du 2026-07-29, base ou aucune ligne ne porte d'horodatage:
 *
 *     newestObservation -> 1999-12-31T23:00Z     detectGap -> un trou de 232 957 h
 *
 * Soit UN TROU FABRIQUE DE 26 ANS, ecrit dans `blackouts.json`, lui-meme commite toutes les heures. Et
 * l'en-tete de ce fichier previent contre exactement ca: « a false gap teaches the reader to discount the
 * real ones ». Le module inventait ce dont il se mefiait.
 *
 * Regle generale, payee ici: TESTER LA FORME AVANT DE PARSER. `Date.parse` accepte tout et rend une date
 * plausible pour des entrees qui n'en sont pas.
 */
function newestObservation(db) {
  const rows = Array.isArray(db) ? db : Object.values(db || {});
  return rows.reduce((m, t) => {
    const brut = t && (t.lastSeen || t.firstSeen);
    if (typeof brut !== 'string' || !brut) return m;   // ni nombre, ni null, ni undefined n'atteint Date.parse
    const v = Date.parse(brut);
    return Number.isFinite(v) && v > 0 && v > m ? v : m;
  }, 0);
}
