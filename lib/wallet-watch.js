...
async function judgeCounterparty(exp, address, lireJson) {
   ...
   * `if (info.is_contract)` traitait « l'explorateur dit non » et « l'explorateur n'a rien dit » de la
   * meme facon. Mesure du 2026-07-28, meme `lireJson` injecte:
   *
   *   {is_contract:false} -> « a plain wallet, not a contract »
   *   {}                  -> « a plain wallet, not a contract »   (IDENTIQUE, severite null)
   *
   * Une reponse 200 a la forme changee faisait donc AFFIRMER la nature d'une adresse que personne n'avait
   * mesuree — et c'est la lecture rassurante qui sortait. Le sommet d'a cote, lui, etait deja correct:
   * `if (!info)` dit explicitement que l'explorateur n'a pas repondu. La prudence s'arretait a la
   * frontiere du corps de reponse.
   *
   * On DIVULGUE sans escalader: « je n'ai pas pu lire » n'est pas « contrat non verifie ». Le second est
   * un fait sur le contrat, le premier une lacune de notre cote, et les confondre serait l'erreur
   * symetrique de celle qu'on corrige.
   *
   * ⚠️ CE QUI PORTE LA DISTINCTION, C'EST LE `=== false`, PAS LE `=== true`. Mesure par mutation: remettre
   * `if (info.is_contract)` a la place de `if (info.is_contract === true)` rend une sortie IDENTIQUE au
   * caractere pres sur les quatre formes de reponse — un `undefined` etant falsy, il retombe de toute
   * facon dans la branche « n'a rien dit ». Le `=== true` est donc redondant, et il reste la parce qu'il
   * rend les trois etats lisibles d'un coup d'oeil. En revanche, remplacer le `=== false` par un `else`
   * ramene immediatement le defaut: ne pas « simplifier » cette ligne-la. */
...
/* Bounded 2026-08-13, same gap as its six siblings in lib/: `on('error')` does not fire for a host
 * that accepts and never answers, and this promise only resolves — so that case hung forever. It
 * matters here because judgeCounterparty already discloses when the local screen is unavailable; a
 * read that never returns would not even reach that disclosure. 12s and the destroying handler from
 * agent-vet.js:158; resolve(null) is the failure this module already models. */
...
async function watchWallet(chain, owner, { persist = true, deps, stateDir } = {}) {
  ...
  /* ⚠️ MEME TROU QUE lib/approvals.js, et ici il ECRIT. `statePath` fabrique son nom de fichier avec
   * `String(owner)`, donc un proprietaire absent produisait `base-undefined.json` — une ligne de base
   * pour personne, dans le seul etat persistant du depot, contre laquelle tous les runs suivants se
   * seraient compares. Et une chaine vide vise l'adresse ZERO, qui est un vrai wallet.
   * Sur un moniteur, une liste d'alertes vide se lit « tu es tranquille »: c'est la phrase qu'il ne faut
   * jamais produire pour une question qu'on n'a pas posee. Refus AVANT toute lecture ou ecriture. */
  ...
  if (!/^0x[0-9a-fA-F]{40}$/.test(proprietaire)) {
    return { ok: false, rejectedInput: true, owner: null,
      reason: 'REJECTED INPUT: `owner` is ' + (proprietaire ? '"' + proprietaire.slice(0, 24) + '"' : 'missing')
        + ', not a 0x address (40 hex). NOTHING WAS WATCHED and no baseline was written — an empty alert '
        + 'list here would read as "nothing changed around your wallet", which was never checked.' };
  }
  const exp = EXPLORER[String(chain).toLowerCase()];
  if (!exp) return { ok: false, reason: 'chain "' + chain + '" not wired' };
  const addr = proprietaire.toLowerCase();
