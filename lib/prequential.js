 ...
 */
/**
 * fenetreAveu — ce que la fenetre de maturite se sait a elle-meme, publie avec les chiffres qu'elle
 * produit.
 *
 * ⛔ TOUT VERDICT « SURVECU » DE CE FICHIER REPOSE SUR `maturityH`, ET SON TAUX D'ERREUR ETAIT CALCULE
 * SANS ETRE LU. `lib/scorecard.js` rend `beyondWindow` — le nombre de rugs plus lents que la fenetre —
 * et les deux graders d'ici ne destructuraient que `maturityH`. Le producteur connaissait la marge
 * d'erreur de son propre instrument; le consommateur a fort enjeu la jetait.
 *
 * Mesure du 2026-08-06 sur 1454 rugs dates: fenetre 4 h (plafond du p95 a 3,72 h), et 71 rugs (4,9 %)
 * l'ont DEPASSEE — ils etaient vivants a 4 h et ont rugge ensuite. Un token etiquete survivant porte
 * donc une chance connue de rugger apres coup.
 *
 * ⚠️ ET LA FENETRE MESURE UNE DETECTION, PAS UNE VIE. `ruggedAt` est l'instant ou le radar a REMARQUE
 * le rug, et il passe toutes les 60 minutes: la distribution des durees de vie a p50 = p75 = 1,02 h,
 * c'est-a-dire exactement un cycle. `maturityH = 4` est donc « quatre cycles de sondage », un trait de
 * l'ORDONNANCEUR et non des tokens. Changer la cadence du radar deplacerait en silence chaque etiquette
 * « survecu » du systeme — le meme genre de derive d'instrument que `siblingCount` a subie, mais posee
 * sous tout le reste.
 */
function fenetreAveu(f) {
  const rugs = Array.isArray(f.lifetimes) ? f.lifetimes.length : 0;
  return {
    maturityH: f.maturityH,
    p95RugH: f.p95RugH == null ? null : +f.p95RugH.toFixed(2),
    slowestRugH: f.slowestRugH == null ? null : +f.slowestRugH.toFixed(1),
    rugsDates: rugs,
    /* Le producteur COMPTE desormais les rugs dont la duree de vie n'a pas pu etre lue (`unreadable`,
     * 2026-08-07); ce consommateur le republie, parce que `rugsDates` seul ne dit pas si le reste du
     * monde etait vide ou illisible. Meme motif que `beyondWindow` ci-dessus: le producteur connaissait
     * le trou, le consommateur a fort enjeu ne le lisait pas. */
    rugsNonDates: f.unreadable == null ? null : f.unreadable,
    rugsAuDela: f.beyondWindow,
    partAuDela: rugs ? +((f.beyondWindow / rugs) * 100).toFixed(1) : null,
    note: rugs
      ? f.beyondWindow + ' rug(s) sur ' + rugs + ' ont dure plus que la fenetre de ' + f.maturityH
        + ' h; un token etiquete « survecu » peut donc encore rugger. La fenetre vient du p95 des '
        + 'durees mesurees entre la premiere vue et la DETECTION du rug — le radar passe toutes les 60 '
        + 'minutes, donc elle compte des cycles de sondage autant que des heures de vie.'
        + (f.unreadable ? ' ⚠️ ' + f.unreadable + ' rug(s) observe(s) n ont AUCUNE duree de vie lisible '
          + 'et n entrent ni au numerateur ni au denominateur ci-dessus.' : '')
      : (f.unreadable
        ? f.unreadable + ' rug(s) observe(s), mais AUCUN n a de duree de vie lisible: la fenetre n a pas '
          + 'pu etre derivee et aucun « survecu » n a de fondement. C est un trou de donnees, pas un '
          + 'registre vierge.'
        : 'aucun rug date: la fenetre ne repose sur rien et aucun « survecu » n a de fondement.'),
  };
}
...
const RULES = [
  ...
  {
    /* ⛔ CETTE REGLE PORTE SA BORNE DANS SA DEFINITION, et c'est tout le sujet.
     *
     * `thin` mesure sur la POPULATION ENTIERE separe de 15,1 points pour 20 disponibles — parce que 83 %
     * de cette population rugge et qu'il n'y reste presque rien a separer. Le meme `thin` mesure DANS la
     * branche « sur » de `funder-20` separe de 69,5 points. Publier le premier chiffre serait publier un
     * plafond; publier le second SANS sa borne serait publier un chiffre qui ne s'applique nulle part.
     * La regle s'abstient donc partout ou elle n'a pas ete mesuree.
     *
     * ⛔ ET ELLE S'ABSTIENT SUR `not_a_candidate` ET `unknown` au lieu de les compter comme « pas thin ».
     * Mesure du 2026-08-09: les mettre dans le complement — ce que fait la lecture canonique ailleurs —
     * coute 3,3 points sur QUATRE lignes dans ce petit terrain. « Pas verifie » n'est pas « verifie et
     * negatif », et sur 242 tokens la difference se paie.
     *
     * CE QUI LA DISTINGUE DES TROIS AUTRES CANDIDATS (mesure du 2026-08-09):
     *   · elle survit au retrait des trois plus gros financeurs (+66,2 -> +32,4, intervalles disjoints),
     *     la ou `jetable` s'effondre et ou `relance` et `impersonation` deviennent indecidables ;
     *   · elle survit a la stratification par `firstLiq` (+60,1 et +53,8 points, disjoints): ce n'est
     *     donc PAS un proxy de petite liquidite, et elle ne meurt pas de ce qui a tue `impersonation`.
     *
     * ⚠️ Tout ca reste IN-SAMPLE. C'est exactement pourquoi elle est annoncee dans `announced-rules.js`
     * et notee vers l'avant: trois regles sont mortes ici de la confusion entre « decrit le passe » et
     * « aurait appele ». */
    key: 'thin-sous-le-seuil',
    label: 'symbole THIN, DANS la branche « sur » de funder-20 (borne dans la definition)',
    derived: false,
    predict(t) {
      if (typeof t.siblingCount !== 'number') return ABSTAIN;      // financeur jamais lu: hors du terrain
      if (t.siblingCount >= 20) return ABSTAIN;                    // au-dessus du seuil: sature, rien a dire
      const v = t.symbolVerdict;
      if (typeof v !== 'string' || v === 'not_a_candidate' || v === 'unknown') return ABSTAIN;
      return v === 'thin' ? DANGER : SAFE;
    },
  },
 ...
 */
/* L'HORLOGE EST UNE ENTREE, ET ELLE ENTRE PAR argv.
 * Les deux runners font `process.argv[2] || new Date().toISOString()`, donc n'importe quelle chaine
 * atteint le `Date.parse` d'ici. Une chaine illisible vaut NaN, et TOUTE comparaison avec NaN est
 * fausse: plus rien ne se resout, tout bascule en « ouvert », et les deux bulletins publient une
 * PHRASE la ou il faudrait un aveu.
 *
 * MESURE DU 2026-08-15, horloge « hier soir », base reelle de 2785 tokens:
 *   run-announced    « 0 appel(s) resolu(s) depuis l annonce — ... aucune conclusion ne se tire »
 *                    alors que 863 appels etaient resolus.
 *   run-prequential  « resolus a hier soir : 0 · non resolus : 2785 »   (reel: 2514 resolus)
 *                    et surtout « fuites vers le futur : 0 » — le controle d'INTEGRITE du protocole
 *                    passe au VERT parce qu'il n'a plus rien a mordre.
 * C'est la direction qui ne se fait jamais questionner: ca porte le costume de la prudence.
 *
 * ⚠️ LA BONNE VERIFICATION EXISTAIT DEJA A TROIS LIGNES: `Number.isFinite(Date.parse(...))`, posee
 * sur les DONNEES (voir les deux filtres ci-dessous) et jamais sur l'horloge qui les juge.
 * On REFUSE au lieu de deviner: une horloge illisible n'est pas « maintenant », et un refus bruyant
 * coute un relancement la ou un chiffre faux se publie. */
/* ⚠️⚠️ `Date.parse` A DEUX PIEGES, ET ILS ECHOUENT DANS DES DIRECTIONS OPPOSEES. Mesure du 2026-08-15:
 *     Date.parse(42)              -> 2041-12-31   un nombre nu est lu comme une ANNEE
 *     Date.parse(0)               -> 1999-12-31   idem
 *     Date.parse(1786000000000)   -> NaN          un vrai epoch-ms est REFUSE
 * C'est l'inverse de l'intuition: la valeur qui RESSEMBLE a un timestamp est rejetee, et celle qui
 * n'a rien d'une date devient un instant seize ans dans le futur. Or une horloge en 2042 rend TOUT
 * resolu — l'echec OUVERT, ou le bulletin publie sa confiance maximale sur une date fabriquee —
 * pendant que NaN rend tout OUVERT au sens inverse. Un seul `Number.isFinite` n'attrape qu'une des
 * deux moities: c'est le tri par DIRECTION, applique a un parseur au lieu d'un comparateur.
 * On n'accepte donc que ce que cette API documente: une chaine ISO-8601, ou un objet Date. */
function horlogeLisible(now, quiAppelle) {
  const refus = (pourquoi) => new TypeError(quiAppelle + ': horloge illisible '
    + JSON.stringify(String(now).slice(0, 40)) + ' (' + pourquoi + ') — attendu une chaine ISO-8601 '
    + '(ex. 2026-08-15T00:00:00.000Z) ou un objet Date. Le refus est delibere: une horloge fausse ne '
    + 'rend pas le bulletin muet, elle le rend AFFIRMATIF — « 0 resolu » ou, pire, tout resolu.');

  if (now instanceof Date) {
    const t = now.getTime();
    if (Number.isFinite(t)) return t;
    throw refus('objet Date invalide');
  }
  if (typeof now !== 'string') throw refus('type ' + typeof now + ', pas une chaine — un nombre nu '
    + 'serait lu comme une ANNEE, et un epoch en millisecondes ne serait pas lu du tout');
  if (!/^\d{4}-\d{2}-\d{2}/.test(now)) throw refus('ne commence pas par une date ISO AAAA-MM-JJ');
  const T = Date.parse(now);
  if (!Number.isFinite(T)) throw refus('Date.parse rend NaN');
  return T;
}

function runPrequential(all, now, opts = {}) {
  /* ⚠️ CE FILTRE JETTE, ET IL LE DISAIT A PERSONNE. Une ligne sans `firstSeen` lisible ne peut pas
   * entrer dans une marche chronologique — le retrait est CORRECT. Ce qui manquait, c'est son COMPTE:
   * les lignes ecartees disparaissaient du denominateur, de l'historique de chaque prediction et de
   * tout chiffre publie, sans laisser de trace. Ce module ecrit lui-meme « La BORNE voyage avec le
   * chiffre » vingt lignes plus bas; c'etait la seule borne qui ne voyageait pas.
   * ⚖️ MESURE DU 2026-08-15 SUR LA BASE REELLE: 2775 lignes, 2775 datables, **0 ecartee**. Le manque est
   * donc LATENT, pas vivant — et c'est precisement pour ca qu'il fallait le publier: un collecteur qui
   * changerait de nom de champ demain ferait fondre la population sans qu'aucun chiffre ne bouge. */
  const toutes = Array.isArray(all) ? all : Object.values(all || {});
  const rows = toutes
    .filter((t) => Number.isFinite(Date.parse(t.firstSeen)))
    .sort((a, b) => Date.parse(a.firstSeen) - Date.parse(b.firstSeen));
  const tokensUndated = toutes.length - rows.length;

  const fenetre = maturityWindow(rows);
  const { maturityH } = fenetre;
  const rules = opts.rules || RULES;
  const T = horlogeLisible(now, 'runPrequential');
  ...
  /* Base rate over every resolved token in the walk.
   *
   * ⛔ CORRECTION DU 2026-08-06 — CETTE LIGNE DISAIT « over the SAME resolved population the rules are
   * graded on », ET C'ETAIT FAUX. Elle n'est vraie que d'une regle qui ne s'abstient JAMAIS. Des qu'une
   * regle s'abstient, sa precision porte sur un sous-ensemble et cette base porte sur le tout — c'est-a-
   * dire exactement « a base rate computed over a different denominator », le piege que la phrase
   * suivante denonce. Cinq des onze regles etaient dans ce cas, jusqu'a 7,1 points d'ecart gratuit.
   * `baseRateJuge` par carte donne la comparaison honnete; celle-ci reste publiee comme reference du
   * terrain, ce qu'elle a toujours vraiment ete. */
  ...
  return {
    ...
    /* La fenetre publie desormais son propre taux d erreur a cote des chiffres qu elle produit. */
    fenetre: fenetreAveu(fenetre),
    tokensWalked: rows.length,
    /* Le compte des lignes ECARTEES faute de date lisible. `tokensWalked` seul se lit comme « toute la
     * base »; les deux ensemble disent quelle part a ete marchee. */
    tokensUndated,
    ...
    cards: [...cards.values()].map((c) => {
      ...
      /* ⛔ LA BASE DE CETTE REGLE, PAS CELLE DE LA BASE. Le commentaire de `resolvedTotal` plus haut
       * affirme que la base est calculee « sur la MEME population resolue que les regles ». C'est vrai
       * d'une regle qui ne s'abstient jamais, et faux de toutes les autres — et une regle qui s'abstient
       * sur les cas faciles gagne des points gratuitement, ce que ce meme commentaire denonce.
       *
       * Mesure du 2026-08-06, precision moins base, selon la base employee:
       *   funder-derived-uncensored   1134 abstentions   +7,1 pts globale   →   -0,0 pt jugee
       *   suffixe-01                  1767 abstentions   +9,1 pts           →   +0,4 pt
       *   owner-unknown                675 abstentions  +12,7 pts           →   +4,6 pts
       *   owner-renounced              675 abstentions  +11,4 pts           →   +3,3 pts
       *   simulation-et-financeur-lu  1026 abstentions  +11,5 pts           →   +8,5 pts
       * Les trois regles a ZERO abstention ne bougent pas d'un centieme, ce qui est le controle.
       *
       * 💎 Rien a accumuler: la population jugee est exactement `dangerResolved + safeResolved`, et ses
       * rugs `dangerRugged + safeRugged`. Les compteurs existaient deja. */
      const bj = tauxPublie(c.dangerRugged + c.safeRugged, dangerResolved + safeResolved);
      return {
        ...c,
        ...
        /* La base des appels REELLEMENT juges, et l'ecart contre elle. `precision - baseRate` compare a
         * une population qui inclut les abstentions; seul `liftVsBaseJuge` dit si la regle bat le
         * hasard LA OU ELLE PARLE. Les deux sont publies parce qu'ils repondent a deux questions. */
        baseRateJuge: bj.rate, baseRateJugeN: bj.n,
        liftVsBaseJuge: (p.rate == null || bj.rate == null) ? null : +((p.rate - bj.rate) * 100).toFixed(1),
...
function gradeAnnounced(all, now, opts = {}) {
  // Le JUMEAU de `runPrequential`: meme filtre, meme silence. Corriger l'un sans l'autre laisserait la
  // moitie publiante sans sa borne — le motif que ce depot passe ses nuits a defaire.
  const toutes = Array.isArray(all) ? all : Object.values(all || {});
  const rows = toutes.filter((t) => Number.isFinite(Date.parse(t.firstSeen)));
  const tokensUndated = toutes.length - rows.length;
  const fenetre = maturityWindow(rows);
  const { maturityH } = fenetre;
  const rules = new Map((opts.rules || RULES).map((r) => [r.key, r]));
  const annonces = opts.announced || require('./announced-rules').ANNOUNCED;
  const T = horlogeLisible(now, 'gradeAnnounced');   // meme garde que le jumeau, posee au meme endroit
  ...
  const cartes = annonces.map((a) => {
    ...
    /* ⛔ UN PARI QUI S'ABSTIENT SUR *TOUTE* SA POPULATION N'EST PAS UN PARI JEUNE: IL EST INERTE.
     *
     * Les deux etats produisaient le meme mot, `trop-peu`, et le meme affichage. Or l'un s'ameliore avec
     * le temps et l'autre JAMAIS — c'est un pari MORT deguise en pari JEUNE, la forme la plus rassurante
     * qu'une panne puisse prendre.
     *
     * Mesure du 2026-08-10: `natif-b20`, pari ANNONCE, rendait 488 abstentions sur 488 eligibles — 100 %.
     * Cause: sa `predict` lit `t.addr`, un champ qui n'existe sur AUCUNE ligne (l'adresse est la CLE de
     * l'objet, pas une propriete de la valeur). `suffixe-01` est dans le meme cas. Et
     * `verdict-caution-sans-natifs` — annonce aussi — voit son exclusion des natifs ne JAMAIS s'appliquer,
     * ce qui le rend identique a `verdict-caution` (1499/848 des deux cotes, mesure).
     *
     * ⚠️ CE VERDICT NE MODIFIE AUCUNE REGLE ET AUCUN COMPTE. Il NOMME un etat qui existait deja et que
     * l'affichage confondait avec un autre. Toucher la regle deplacerait les poteaux d'un pari annonce;
     * nommer sa panne ne le fait pas. */
    if (abstained === eligibles.length) {
      return { ...base, verdict: 'inerte',
        note: 'la regle s abstient sur la TOTALITE de ses ' + eligibles.length + ' eligibles — ce n est pas '
          + '« pas encore assez de donnees », c est « elle ne peut rien dire sur cette population ». '
          + 'Verifier que les champs qu elle lit existent bien sur les lignes: une regle qui interroge '
          + 'une propriete absente s abstient poliment et pour toujours.' };
    }
    ...
    return { ...base, verdict: 'note', observed, tropMince,
      ...
      /* ⛔ LE PLAFOND VOYAGE AVEC L'ECART, SANS QUOI UN ZERO SE LIT COMME UN ECHEC. Sur une population
       * qui rugge a 97,2 %, l'ecart maximum atteignable du cote danger est 2,8 points; a 100 %, il est
       * NUL — aucune regle, si parfaite soit-elle, ne peut y montrer quoi que ce soit.
       *
       * Le 2026-08-06 les quatre cotes publiables sortaient tous entre -0,7 et +0,0 point contre leur
       * base, et il etait tentant d'en conclure que rien n'avait ete prouve. C'est vrai qu'aucune preuve
       * n'a ete produite, mais la RAISON n'est pas que les regles echouent: `owner-renounced` jugeait 26
       * appels dont 26 avaient rugge, plafond zero. Publier l'ecart sans le plafond accuserait une regle
       * d'un resultat que la population rendait impossible — l'inverse exact du sur-claim, et tout aussi
       * faux. */
      plafondEcart: baseTx.rate == null ? null : {
        danger: +((1 - baseTx.rate) * 100).toFixed(1),      // precision ne peut pas depasser 100 %
        safe: +(-baseTx.rate * 100).toFixed(1),             // un cote sur parfait rugge a 0 %
      },
  ...
  return { updatedAt: now, maturityWindowHours: maturityH, fenetre: fenetreAveu(fenetre),
    tokensAvailable: rows.length, tokensUndated, cards: cartes,
...
module.exports = { runPrequential, gradeAnnounced, outcomeKnownAt, maturityWindow, quantile, coupeQuartiles,
  tauxPublie, MIN_RESOLUS, instrumentTemoin, fenetreAveu, RULES, DANGER, SAFE, ABSTAIN };
