...
const microToUsd = (micro) => {
  ...
  return `${m / 1_000_000n}.${(m % 1_000_000n).toString().padStart(6, '0').replace(/0+$/, '') || '00'}`;
};
...
function verifyPayment(charge, fact) {
  ...
  const got = BigInt(String(fact.valueMicro || '0')), want = BigInt(charge.amountMicro);
  if (got < want) return { paid: false, reason: `underpaid: got ${microToUsd(got.toString())} of ${charge.amountUsd} USDC` };
  const conf = Number(fact.confirmations) || 0;
