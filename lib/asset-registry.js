...
/**
 * loadAssetRegistry({dir?}) → { entries: [...]|null, source: string|null, complete: true|false|null }.
 * dir injectable for tests.
 *
 * ⚠️ `complete` IS THREE-STATE, and the third state is the point. `true` = the ingest ran to the end;
 * `false` = it lost pages or categories; `null` = UNKNOWN — a file written before the flag existed, or
 * no aggregator file at all. Reading `agg.complete !== false` would have folded that `null` into "yes,
 * complete", which is precisely the failed-read-looks-like-a-good-read shape this flag exists to break.
 * Only an explicit `true` may ever license accusing a contract on its absence from the registry.
 */
function loadAssetRegistry({ dir = DATA_DIR } = {}) {
  ...
  if (!issuerEntries.length && !aggEntries.length) return { entries: null, source: null, complete: null, incomplete: null };
  ...
  // Only a literal `true` counts as complete. Anything else — false, missing, a stray string — is UNKNOWN.
  const complete = agg && typeof agg.complete === 'boolean' ? agg.complete : null;
  return { entries, source: parts.join(' + '), complete, incomplete: (agg && agg.incomplete) || null };
}
