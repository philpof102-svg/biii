...
function assess(payments, actors = {}, opts = {}) {
  ...
  return {
    ...
    /* ⛔ TROIS ETATS, PAS DEUX — ET ICI LE ZERO FABRIQUE ACCUSE au lieu de rassurer.
     *
     * Ces deux parts rendaient `0` quand leur denominateur etait vide. `provenShare: 0` se lit « aucune
     * livraison de cet acteur n'est prouvee », ce qui est un reproche; la verite est « il n'y avait
     * aucune livraison a prouver », ce qui n'en est pas un. Accuser sur notre propre absence de donnees
     * est la faute que ce depot s'interdit explicitement ailleurs.
     *
     * ⚠️ LE TEST EPINGLAIT CE CHOIX, ET SON COMMENTAIRE DIT POURQUOI: « couverture 0 sur vide, jamais
     * 1 ». L'auteur arbitrait entre 0 et 1 et a pris le plus prudent des deux — eviter de pretendre
     * qu'on a tout couvert. Le bon reflexe, sur une liste trop courte: `0/0` n'est pas zero, il est
     * INDEFINI, et la troisieme issue n'etait pas sur la table.
     *
     * ⚠️ ET LE VRAI ZERO RESTE UN ZERO. Une arete comptee dont la livraison n'est pas prouvee donne
     * `0/1`, qui vaut bien 0 et doit se voir — c'est ce que verifie le cas « part prouvee nulle, et elle
     * doit se voir ». Seul le denominateur VIDE change de reponse. Les deux denominateurs voyagent
     * desormais avec leur part, pour qu'un `null` ne soit pas muet. */
    delivery: { served, unverifiable, substituted,
      provenShare: kept.length ? served / kept.length : null,
      provenOf: kept.length },
    coverage: edges.length ? counted.length / edges.length : null,
    coverageOf: edges.length,
    disclosure: 'La reputation ne compte QUE les paiements entre acteurs dont l independance est etablie. '
      ...
      + '`provenShare` et `coverage` valent null quand leur denominateur est VIDE — rien a mesurer n est '
      + 'un troisieme etat, distinct de « mesure et nul ». Leurs denominateurs sont publies a cote '
      + '(`provenOf`, `coverageOf`) pour qu un null ne soit jamais muet.',
  };
}
