...
function scanKeyPaths(paths, { maxFiles = 40000 } = {}) {
  ...
  return {
    ...
    secrets, keystores,
    // THE SCOPE OF THIS OBJECT IS NOT UNIFORM, and that has to be a field rather than a sentence.
    // `secrets` and `keystores` describe the paths in `scanned`. `vaults` describes THE MACHINE, on
    // purpose: a browser-extension wallet keeps its vault in a fixed profile directory, so scoping it
    // to the caller's path would answer "no wallet" for every scan that did not happen to include the
    // browser profile — a clean result carrying no information. But a caller reading one JSON blob
    // assumes one scope, and prose in `disclosure` does not reach a consumer that reads fields.
    // Measured 2026-07-27: auditing an extracted npm tarball reported eight vaults. All eight were
    // this machine's browser wallets. None were in the tarball. The number was right and the reading
    // was wrong, which is the failure that matters.
    vaults: findVaults(),
    scope: { secrets: 'paths', keystores: 'paths', retained: 'paths', vaults: 'machine' },
    ...
    disclosure: 'No key material appears in this output, by design — fingerprints are truncated hashes and ' +
      ...
      'remove the editor-history and backup snapshots of what the file used to say. READ `scope` BEFORE ' +
      'READING ANY COUNT: `secrets` and `keystores` cover only the paths given, while `vaults` covers the ' +
      'whole machine and will report browser wallets that are nowhere near those paths. Vaults are reported ' +
