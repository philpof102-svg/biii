...
async function vetMeme(opts = {}) {
  ...
  let source = typeof freres === 'number' ? 'fourni par l appelant' : null;
  let vu = null;
  if (typeof freres !== 'number') {
    ...
    if (lu.state === 'hit' && typeof lu.siblingCount === 'number') {
      freres = lu.siblingCount; source = 'observation de ce noeud'; vu = lu.observedAt;
    } else {
  ...
  const risque = observationThin({ status: avecB20.status, siblingCount: freres,
    gradedForward: opts.gradedForward });
  return { ...avecB20,
