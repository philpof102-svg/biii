 ...
 */
/* ⛔ UNE URL VENUE D'UNE API TIERCE FINIT DANS UN href, ET L'ECHAPPEMENT HTML NE REGARDE PAS LE SCHEMA.
 * `web/vet-meme.html` rend ce champ en <a href="…"> apres l'avoir passe a son `esc()` — qui neutralise
 * & < > " et RIEN D'AUTRE. Un `javascript:` traverse l'echappement INTACT et s'execute au clic: on ne
 * casse pas l'attribut, on remplace le schema, et aucun echappeur HTML ne voit la difference.
 * Le champ etait repris tel quel de la reponse DexScreener (`p.url`), avec une URL construite en repli.
 * DexScreener n'a aucune raison d'envoyer autre chose que du https — et c'est exactement le probleme:
 * l'hypothese n'etait ecrite NULLE PART, donc rien ne la defendait. On n'accepte donc de l'API que ce
 * dont le schema est http(s), et on retombe sinon sur l'URL que NOUS construisons, dont on connait le
 * schema par construction. Une valeur inutilisable vaut mieux qu'une valeur executable. */
const urlSure = (u, repli) => {
  const s = typeof u === 'string' ? u.trim() : '';
  return /^https?:\/\//i.test(s) ? s : repli;
};
...
function candidatesFrom(pairs, symbol, chainId) {
  ...
  for (const p of (pairs || [])) {
    ...
    /* ⛔ ON NORMALISE POUR FILTRER ET ON STOCKAIT BRUT — ET LA COUCHE 2 EST GARDEE PAR UN `===`.
     * Le filtre ci-dessus compare `String(p.chainId).toLowerCase()`, donc une paire annoncee 'Base'
     * ou 'BASE' passe. Mais `chain` gardait la casse de l'API, et la couche holders-health est
     * gardee ligne ~202 par `canonical.chain === 'base'` — comparaison SENSIBLE A LA CASSE.
     * MESURE DU 2026-08-15, de bout en bout, meme paire, seule la casse change:
     *     chainId 'base' -> holders-health APPELEE     verdict genuine
     *     chainId 'Base' -> holders-health JAMAIS appelee, verdict genuine QUAND MEME
     * La couche de detection de rug disparaissait en silence, et le verdict restait rassurant.
     * ⚖️ L'en-tete de ce fichier documente que cette meme couche a DEJA ete morte une fois, gardee
     * par ce meme `=== 'base'` (slug vs nombre, corrige le 2026-07-27). Deuxieme forme, meme garde.
     * ⚠️ BORNE HONNETE: je n'ai PAS observe DexScreener rendre 'Base'. Le defaut est LATENT — ce qui
     * le rend reel, c'est qu'on normalise deja d'un cote et pas de l'autre, et que rien n'ecrit
     * l'hypothese. On stocke donc ce que les comparaisons attendent. */
    if (mieux) byTok[k] = {
      chain: String(p.chainId || '').toLowerCase(), address: p.baseToken.address, name: p.baseToken.name || '',
      liquidityUsd: liq, volume24: (p.volume && p.volume.h24) || 0, pairCreatedAt: p.pairCreatedAt || 0,
      dexscreener: urlSure(p.url, 'https://dexscreener.com/' + p.chainId + '/' + p.baseToken.address),
    };
  }
...
async function vetMeme(opts = {}) {
  ...
  if (typeof freres !== 'number') {
    /* ⛔ `canonical` EST UN OBJET, PAS UNE ADRESSE — et `readingFor` n'accepte qu'une chaine.
     * Ce repli passait `{chain, address, liquidityUsd, dexscreener}` la ou une adresse etait attendue,
     * donc il tombait sur la garde d'entree et rendait `state:'miss'` SANS jamais ouvrir la base.
     *
     * Mesure du 2026-08-19, meme token, meme base fabriquee, meme canonique `0xaaaa…aaaa`:
     *   avec `address` fourni ... siblingCount=3   boundChecked=true   rate=0.189
     *   sans `address`       ... siblingCount=undef boundChecked=false  rate=null
     * Or `address` est OPTIONNEL sur `till_vet_meme` et sa description promet exactement l'inverse:
     * « omit it and this node checks its own observation database for free ». Le chemin gratuit que la
     * fiche vend etait donc le seul ou la base n'etait jamais consultee, et la retenue tenait POUR
     * TOUJOURS sur un token que ce noeud avait deja observe.
     *
     * ⚠️ ET LE REFUS ACCUSAIT L'APPELANT. La source servie disait « not a 0x address of 40 hex —
     * nothing was looked up », c'est-a-dire une faute dans une entree que l'appelant n'avait PAS
     * fournie, alors que le canonique etait une adresse parfaitement valide. Notre cablage rate se
     * lisait comme sa faute de frappe — never-accuse-on-own-incompleteness, dans une route servie.
     *
     * ⛔ Le VERDICT ne bouge pas: `status`, `canonical` et `candidates` sont calcules en amont et ne
     * dependent pas de cette lecture. Seule la retenue sur `observedRisk` se leve quand elle le doit. */
    const canonAdr = avecB20.canonical && avecB20.canonical.address;
    const cible = typeof opts.address === 'string' ? opts.address : canonAdr;
    ...
    /* ⛔ TROIS CAS, PAS DEUX. Sans adresse fournie ET sans canonique (thin / ambiguous / unknown), il
     * n'y a rien a chercher — et le dire ainsi, plutot que de laisser `readingFor` rendre sa phrase
     * d'entree malformee, qui decrirait une faute que personne n'a commise. */
    const lu = typeof cible === 'string' ? readingFor(cible, { dbPath: opts.dbPath })
      : { state: 'not_looked_up', siblingCount: null, detail: 'no address to look up — the caller named '
          + 'none and no single canonical contract was identified, so the observation database of this node '
          + 'was NOT consulted. That is a gap in this answer, not a finding about the token.' };
