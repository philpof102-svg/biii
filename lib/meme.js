  ...
  if (address) {
    ...
    if (!/^0x[0-9a-f]{40}$/.test(a)) {
      return { status: 'unknown', reason: 'the address to judge is not a 0x address (40 hex) — nothing is '
        ...
        canonical, candidates: credible.slice(0, 12), disclosure, health, ...mesure };
    }
    ...
    if (!asCand) {
      return { status: 'not_a_candidate', reason: 'this address does not carry the symbol "' + symbol
        ...
        candidates: credible.slice(0, 12), disclosure, health, ...mesure };
    }
    return { status: 'impersonation', reason: 'this is NOT the dominant contract — the real one holds far more liquidity', canonical, thisContract: asCand, candidates: credible.slice(0, 12), disclosure, health, ...mesure };
  }
