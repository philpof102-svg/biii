 ...
 * @returns { status, reason, canonical, candidates, disclosure }
 *   status: 'genuine' | 'impersonation' | 'ambiguous' | 'thin' | 'unknown'
 */
async function vetMeme({ symbol, chainId, address, fetchImpl } = {}) {
  ...
  if (address) {
    ...
    if (!clearWinner) return { status: 'ambiguous', reason: 'the top contracts are within ' + DOMINANCE + 'x liquidity of each other — cannot certify a single real one', canonical: null, candidates: credible.slice(0, 12), disclosure };
    if (isTop) return { status: 'genuine', reason: 'this IS the dominant-liquidity contract for "' + symbol + '"', canonical, candidates: credible.slice(0, 12), disclosure };
    const asCand = cands.find((c) => c.address.toLowerCase() === a);
    return { status: 'impersonation', reason: 'this is NOT the dominant contract — the real one holds far more liquidity', canonical, thisContract: asCand || { address, note: 'not found in credible set' }, candidates: credible.slice(0, 12), disclosure };
  }

  // No specific address: report which is canonical, or abstain.
  if (!clearWinner) return { status: 'ambiguous', reason: credible.length + ' contracts, top two within ' + DOMINANCE + 'x liquidity — no single real one can be certified', canonical: null, candidates: credible.slice(0, 12), disclosure };
  return { status: 'genuine', reason: 'one contract dominates for "' + symbol + '"', canonical, candidates: credible.slice(0, 12), disclosure };
}
