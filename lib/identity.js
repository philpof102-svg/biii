...
function bindingLens(attestation, { now = Date.now(), requireExpiry = false } = {}) {
  ...
  /* ⚠️ `reVerify.check` EST UNE CONSIGNE, et elle sortait OCTET POUR OCTET IDENTIQUE sur un refus.
   * Mesuré le 2026-07-30 :
   *   bindingLens({})  -> bound:false, message = 85 caracteres, en-tete canonique `BIII-IDENTITY-BINDING-v1`,
   *                       CONSTANT d'un appel a l'autre, sans nonce, expiry 0
   *                       reVerify.check = « reconstruct `message` and verify sigBase … »
   *                       sigNostr/sigDid/sigBase = null null null
   * Le refus rendait donc un message signable fabrique AVANT toute validation — sur une adresse
   * malformee il place le texte brut de l'appelant (`base: pas-une-adresse`) dans une structure
   * canonique versionnee — et il disait a l'appelant de verifier des signatures inexistantes.
   *
   * C'est l'ASYMETRIE INTRA-FONCTION que ce fichier documente deja deux fois. La ligne « no nonce — an
   * unnonced binding is replayable, refusing » nomme exactement le risque que l'objet du refus
   * reproduisait : une chaine canonique, sans nonce, qui n'expire jamais, livree avec « verifie-moi ».
   *
   * On NE retire PAS `message` : il reste utile pour voir CE QUI aurait ete signe, et le supprimer
   * casserait les appelants. On retire la CONSIGNE qui presuppose un artefact valide. `bound:false`
   * etait deja honnete ; c'est le champ d'a cote qui ne l'etait pas. */
  const refuse = (reason) => ({ ...base, bound: false, reason,
    reVerify: { ...reVerify, check:
      'REFUSED — do NOT sign `message`. It was assembled from the input exactly as received, BEFORE any '
      + 'validation ran, so it is not a canonical binding message: an empty or malformed call still yields '
      + 'a constant, nonce-free, expiry-0 string — the very shape this module refuses as replayable when a '
      + 'caller submits it. Nothing above is attested. Fix the attestation and call again; only a bound:true '
      + 'result carries a message worth verifying.' } });
  ...
  /* ⛔ TROIS ETATS, ET LE TROISIEME MANQUAIT — le jumeau `lib/skyfire.js` l'a corrige le 2026-07-28 et
   * le correctif n'a jamais traverse. Ce fichier lisait `Number(a.expiry) > 0`, donc une expiration
   * PRESENTE mais ILLISIBLE devenait indiscernable d'une expiration ABSENTE. Mesure du 2026-08-15,
   * attestation par ailleurs complete et verified:true:
   *
   ...
   *
   * ⚠️ `'2026-01-01'` est exactement la faute plausible que ce fichier documente DEJA sous une autre
   * forme vingt lignes plus haut (millisecondes au lieu de secondes): un integrateur qui ecrit une date
   * lisible par un humain. Le garde des millisecondes l'attrapait, celui-ci la laissait passer.
   *
   * ⚖️ `expiry: 0` reste « pas d'expiration » — c'est le defaut documente de `bindingMessage`, un CHOIX
   * de l'appelant, pas une illisibilite. Seul un champ FOURNI et non lisible est le troisieme etat. */
  const expBrut = a.expiry;
  const expPresent = expBrut != null && expBrut !== '' && expBrut !== 0 && expBrut !== '0';
  const expNum = Number(expBrut);
  const expLisible = expPresent && Number.isFinite(expNum) && expNum > 0;
  const expIllisible = expPresent && !expLisible;

  if (expLisible && expNum > 1e11) return refuse('expiry ' + a.expiry + ' is not unix SECONDS — it looks like '
    + 'milliseconds. Refusing rather than granting a binding that could never expire.');
  /* Un champ illisible est REFUSE, jamais reinterprete en « permanent ». Accorder l'eternite sur une
   * valeur qu'on n'a pas su lire est precisement ce que la garde des millisecondes ci-dessus refuse de
   * faire, et le raisonnement ne change pas parce que la valeur est illisible plutot que mal unitee. */
  if (expIllisible) return refuse('expiry ' + JSON.stringify(String(expBrut).slice(0, 40)) + ' is not a readable '
    + 'unix-SECONDS number — refusing rather than reading an unreadable expiry as "no expiry". '
    + 'A date string is not an expiry: pass Math.floor(Date.parse(d) / 1000).');
  if (expLisible && now > expNum * 1000) return refuse('binding expired — a stale binding is not a live one, refusing.');
