    ...
    if (req.method === 'POST' && (url.pathname === '/x402/vet-asset' || url.pathname === '/x402/vet-address' || url.pathname === '/x402/vet-meme')) {
      ...
      try {
        ...
        if (url.pathname === '/x402/vet-meme') {
          ...
          const result = await vetMeme({ symbol: b.symbol, chainId: b.chainId, address: b.address, fetchImpl: deps.fetchImpl });
...
if (require.main === module) {
   ...
   * `data/token-radar/blackouts.json`: 68,9 % de couverture, 101,2 h aveugles sur 325,5 h, quatre trous
   * de 11 a 22 h en cinq jours. Le lanceur local le dit lui-meme — « Close them to stop ». Un tiers des
