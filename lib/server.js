...
const STATIC = { '/': 'index.html', '/index.html': 'index.html', '/qrcode.min.js': 'qrcode.min.js', '/manifest.json': 'manifest.json',
  '/embed.js': 'embed.js', '/embed-demo.html': 'embed-demo.html', '/radar.html': 'radar.html', '/p2p.html': 'p2p.html' };
...
function build(deps = {}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    // POST /x402/vet-asset | /x402/vet-address — the PAID verdicts. BIII is the MERCHANT that RECEIVES USDC;
    ...
    if (req.method === 'POST' && (url.pathname === '/x402/vet-asset' || url.pathname === '/x402/vet-address')) {
      const origin = (req.headers['x-forwarded-proto'] || 'https') + '://' + (req.headers.host || '');
      ...
      try {
        ...
        const need = BigInt(priceMicro(process.env.BIII_VET_PRICE_USD || '0.002'));
        if (!proof.paid || proof.to !== merchant || BigInt(proof.valueMicro || '0') < need) {
          const { headers, body } = challenge402({ origin, merchant });
          res.writeHead(402, headers); return res.end(JSON.stringify({ ...body, error: 'payment not verified: need a confirmed USDC payment TO the merchant ≥ the price' }));
        }
        const b = await readBody(req);
      ...
      } catch (e) { return json(res, 502, { error: 'settlement read failed: ' + e.message }); }
    }
