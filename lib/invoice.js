...
function normalizeItem(it) {
  ...
  if (it.qty != null || it.unitUsd != null) {
    ...
    const unitMicro = BigInt(T.usdToMicro(it.unitUsd, `line "${description}": unitUsd`));
    const amountMicro = (unitMicro * BigInt(qty)).toString();
    /* ⚠️ DEUX CHAMPS QUI SE CONTREDISENT NE SE TRANCHENT PAS EN SILENCE.
     * Mesure du 2026-07-28: `{qty:2, unitUsd:'5.00', amountUsd:'100.00'}` rendait 10,00 $ et jetait le
     * 100,00 sans un mot. Les deux valeurs different d'un facteur dix, et le client etait facture sur
     * celle que le code avait choisie tout seul.
     *
     * L'appelant qui envoie les deux a calcule un total quelque part; s'il ne correspond pas au produit,
     * l'un des deux est un bug CHEZ LUI. En choisir un revient a decider a sa place quel montant reclamer
     * a un tiers — c'est la seule chose qu'une couche de facturation ne doit jamais faire seule.
     *
     * ⚠️ Les DEUX bornes: une redondance COHERENTE reste acceptee. Refuser `{qty:2, unitUsd:'5',
     * amountUsd:'10'}` casserait des appelants qui se relisent, et un fail-closed qui refuse le correct
     * cesse d'informer. On ne refuse que la contradiction. */
    if (it.amountUsd != null && it.amountUsd !== '') {
      const annonce = T.usdToMicro(it.amountUsd, `line "${description}": amountUsd`); // jette deja sur une forme illisible
      if (String(annonce) !== amountMicro) {
        throw new Error(`line "${description}": amountUsd ${T.microToUsd(String(annonce))} contradicts `
          + `qty ${qty} x unitUsd ${T.microToUsd(unitMicro.toString())} = ${T.microToUsd(amountMicro)}. `
          + 'Refusing to pick one: send whichever is right, not both.');
      }
    }
    return { description, qty, unitMicro: unitMicro.toString(), amountMicro, amountUsd: T.microToUsd(amountMicro) };
  }
  const amountMicro = T.usdToMicro(it.amountUsd, `line "${description}": amountUsd`); // throws on bad input — fail-closed like the core
 ...
 * { to, lineItems[], number?, issueDateMs?, dueDateMs?, billTo?, merchantName?, orderId?, nowMs?, rail? }
 * The total is the exact micro sum of the line items; the underlying charge is what actually gets paid.
 *
 * `rail` — comment cette facture se REGLE. Absent ou 'base' = USDC sur Base, que BIII lit et sait donc
 * constater. Tout autre nom ('carte', 'sepa', 'especes', 'mastercard-agent-pay') fait rendre
 * `not_observable` a invoiceStatus au lieu d'un impaye: le reglement a peut-etre parfaitement eu lieu,
 * il n'apparaitra simplement jamais sur Base.
 *
 * ⚠️ CE PARAMETRE MANQUAIT PENDANT DEUX HEURES. `invoiceStatus` lisait `invoice.rail` et rien ne le
 * posait: le champ n'existait sur aucune facture creee par ce module. Troisieme fois dans la journee que
 * j'ecris le LECTEUR avant l'ECRIVAIN (apres FRESH_FUNDING_WINDOW_MS dans feeder, et l'etat
 * not_observable de trust.js). Et mes tests ne l'ont pas vu parce qu'ils fabriquaient l'objet A LA MAIN
 * — `{ dueDateMs, rail }` — donc ils ne pouvaient pas detecter que cet objet etait inconstructible en
 * production. Un test qui construit son entree lui-meme ne prouve pas que l'entree existe.
 */
function createInvoice({ to, lineItems, number, issueDateMs, dueDateMs, billTo, merchantName, orderId, nowMs, rail } = {}) {
  ...
  return {
    ...
    /* Le rail NORMALISE une seule fois, ici, par la meme fonction que partout ailleurs (lib/till.js).
     * `null` = aucun rail declare, ce qui laisse invoiceStatus a son comportement d'origine. */
...
function invoiceStatus(invoice, verdict, nowMs) {
  ...
  /* Une seule definition de « temoignable », dans lib/till.js — elle a existe en double pendant deux
   * heures aujourd'hui, avec sa normalisation dupliquee. */
  const { named, rail, witnessable } = T.railOf(invoice && invoice.rail);
  if (named && !witnessable) {
