...
function startRadarTicks(opts = {}) {
  ...
  /* ⛔ L'ETAT VIVANT, PARCE QUE LES LOGS NE SORTENT PAS DU CONTENEUR.
   * Mesure du 2026-08-11: `blackouts.json` porte 111,3 h de silence en 9 trous, tous du meme profil —
   * arret le soir/la nuit, reprise en JOURNEE, et la note du plus ancien dit « The machine slept ».
   * Ce module existe pour que la collecte ne depende plus d'une machine qui dort. Mais il ne DISAIT son
   * etat que par `log()`, donc de l'exterieur personne ne pouvait savoir s'il tournait — et le doute a
   * dure des jours. `etat()` rend cette information lisible par un endpoint.
   * ⚠️ `actif` NE SUFFIT PAS: il dit « arme », pas « tourne ». Un collecteur qui plante a chaque tick
   * resterait `actif:true`. D'ou la date de la DERNIERE TENTATIVE, son ISSUE, et l'AGE — c'est l'age
   * qui montre un collecteur mort. TROIS etats distincts, jamais fondus:
   *   lastTickOk === null  -> jamais tourne        lastTickAt === null
   *   lastTickOk === true  -> tourne et reussi     lastTickAt = date
   *   lastTickOk === false -> tourne et ECHOUE     lastTickAt = date (la TENTATIVE est datee, sinon un
   *                           radar qui plante en boucle passerait pour « jamais lance »). */
  let lastTickAt = null, lastTickOk = null, ticks = 0, lastRaison = null;
  const etatDe = (actif, raison, min, dir) => () => ({
    actif, raison, minutes: min, dir,
    lastTickAt, lastTickOk, ticks, lastTickRaison: lastRaison,
    lastTickAgeMinutes: lastTickAt ? (Date.now() - Date.parse(lastTickAt)) / 60000 : null,
  });
  ...
  if (!decision.run) {
    ...
    return { actif: false, raison: decision.raison, minutes: null, dir: null, stop: () => {},
      etat: etatDe(false, decision.raison, null, null) };
  }
  ...
  const tick = async () => {
    ...
    running = true;
    const t0 = Date.now();
    /* La TENTATIVE est datee AVANT de savoir si elle reussit: un radar qui jette a chaque fois doit
     * apparaitre comme « tourne et echoue », jamais comme « jamais lance ». */
    ticks++;
    lastTickAt = new Date(t0).toISOString();
    try {
      const r = await lancer();
      lastTickOk = r && r.code === 0;
      lastRaison = lastTickOk ? 'ok' : 'code ' + (r && r.code);
      ...
      lastTickOk = false;
      lastRaison = String((e && e.message) || e).slice(0, 120);
  ...
  return { actif: true, raison: 'ok', minutes: m, dir: plan.dir, rattrapage: d0,
    stop: () => clearInterval(h), _tick: tick, etat: etatDe(true, 'ok', m, plan.dir) };
}
