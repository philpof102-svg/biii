...
function proportionAvecBornes(k, n, opts = {}) {
  ...
  /* ⛔ UN EFFECTIF DE ZERO N'EST PAS « PAS D'EFFECTIF ». La premiere version de ce module ecartait tout
   * effectif non strictement positif et retombait sur `n` — donc sur le compte d'OBSERVATIONS, l'estimation
   * la plus etroite et la plus rassurante. Un appelant qui passe 0 vient de constater qu'il n'a identifie
   * AUCUN tirage; lui rendre l'intervalle le plus confiant est le contraire de ce qu'il demande.
   * Attrape le 2026-08-07 en lisant la sortie d'une sonde: « 0 financeur(s) » et « 10 tirage(s) » sur la
   * meme carte. `null`/absent garde son sens — aucune information de groupement, on compte sur n. */
  if (effectif !== null && effectif !== undefined
      && (!Number.isInteger(effectif) || effectif <= 0)) {
    return { taux: null, basse: null, haute: null, k, n, effectif, retenu: false,
      raison: 'effectif annonce a ' + String(effectif) + ' pour ' + n + ' observation(s) — contradictoire '
        + 'ou inconnu; aucune proportion ne se publie, et surtout pas celle calculee sur n' };
  }
