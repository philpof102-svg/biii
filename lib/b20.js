...
/* 2026-08-13: this RPC call had NO deadline, and it is the one that reads chain state to judge a B20
 * token. `on('error')` covers a refused or broken connection; it does NOT cover an endpoint that
 * accepts and never answers, and since this promise only ever resolves, that case left it pending
 * forever. A verdict that never arrives is not fail-closed — it is no verdict at all, indefinitely,
 * which is worse than the honest `null` this function already returns on unparseable output.
 * Measured on a local server that accepts and stays silent: without `timeout` still hanging at 2.5s
 * where 700ms was asked; with `on('timeout', () => req.destroy())` it gave up at 712ms; a handler that
 * does NOT destroy hangs identically. Shape copied from agent-vet.js:158 in this same directory. */
function rpc(url, method, params, timeout = 12000) {
  const body = JSON.stringify({ jsonrpc: '2.0', id: 1, method, params });
  return new Promise((resolve) => {
    const u = new URL(url);
    const req = https.request({ hostname: u.hostname, path: u.pathname, method: 'POST',
      headers: { 'content-type': 'application/json', 'content-length': Buffer.byteLength(body) }, timeout }, (res) => {
