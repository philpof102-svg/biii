...
const post = (url, body, timeout = 12000) => new Promise((resolve) => {
  ...
  const u = new URL(url);
  /* ⚠️ LE PORT ETAIT JETE, ET LE SCHEMA IMPOSE. `https.request({hostname, path, method})` sans `port`
   * part sur 443: une URL `https://exemple.tld:8443/mcp` etait sondee sur 443, et le verdict rendu
   * decrivait un AUTRE service — un fait attribue au mauvais endpoint, ce qui est pire qu'une absence de
   * fait. De meme, une URL `http://` etait attaquee en TLS: l'echec se rapportait « n'a pas repondu »
   * alors que la verite est « on a parle le mauvais protocole ». Meme famille que tout le reste ici: une
   * sonde mal dirigee presentee comme une observation.
   *
   * Mesure du 2026-07-28 sur les 79 endpoints suivis: 0 porte un port explicite — LATENT, pas actif. Le
   * balayage du registre filtre deja sur `^https://`, donc rien n'est ouvert au clair par ce changement:
   * on honore l'URL qu'on nous donne au lieu d'en sonder une autre. */
  const transport = u.protocol === 'http:' ? http : https;
  const req = transport.request({ hostname: u.hostname, port: u.port || undefined,
...
function auditTools(tools) {
  ...
  /* ⚠️ SEPTIÈME SEAU — un nom qu'on ne sait pas LIRE n'est pas un nom qu'on a lu et jugé inoffensif.
   * `tokenize` filtre sur [^A-Za-z0-9], donc un nom sans caractère latin rend `[]`, aucun VALUE_VERB ne
   * peut matcher, et l'outil tombait dans `readOnly` — une AFFIRMATION (« ne déplace pas d'argent »).
   * Mesuré le 2026-07-30, sonde à témoins opposés (4/6 sorties distinctes):
   *   sendPayment -> ["send","payment"]   transferAmount -> ["transfer","amount"]   (témoins+)
   *   送金 -> []    перевод -> []    💸 -> []    送金Amount -> ["amount"]
   * Un outil nommé 送金 — littéralement « envoyer de l'argent » — était classé lecture seule.
   * C'est une limite de NOTRE lecteur encodée comme une propriété du SUJET, exactement comme
   * `siblingCount: 0` sur un financeur jamais identifié. */
  ...
  for (const t of tools || []) {
    ...
    const jetons = tokenize(name);
    const verb = jetons.find((w) => VALUE_VERBS.has(w));
    /* Ce que le tokenizer a SILENCIEUSEMENT JETÉ. Les séparateurs usuels ne comptent pas: mesuré, ils ne
     * laissent aucun résidu (`send_payment`, `send-payment`, `send.payment`, `transfer amount` rendent
     * tous ["send","payment"] et zéro reste). Seuls les caractères porteurs de SENS que notre matcher
     * latin ne sait pas lire en laissent un — CJK, cyrillique, emoji. La frontière est donc fiable. */
    const efface = [...String(name || '')].filter((c) => !/[A-Za-z0-9]/.test(c) && !/[_\-. ]/.test(c)).join('');

    /* Un nom ILLISIBLE ne peut pas être classé lecture seule: on n'a pas cherché de verbe dedans, on n'a
     * rien pu y chercher du tout. La distinction est faite AVANT `readOnly`, sinon l'absence de verbe se
     * lit comme l'absence de danger.
     *
     * DEUX FORMES, et la seconde était la limite écrite ici ce matin, maintenant fermée:
     *   · rien de lisible du tout   — 送金, перевод, 💸           -> jetons vides
     *   · lisible en PARTIE, et le verbe manque — 送金Amount      -> ["amount"], résidu "送金"
     * Le second cas retombait dans `readOnly`. Or c'est précisément le VERBE qui a pu être effacé
     * (送金 = « envoyer de l'argent ») pendant que le CHAMP survivait: la sur-affirmation était la même,
     * juste plus discrète, et une affirmation discrète est plus dure à contester.
     *
     * ⚠️ ASYMÉTRIE VOULUE: si un verbe EST trouvé malgré un résidu (`tip_送金`), on garde le classement.
     * Un verbe trouvé est une preuve POSITIVE, et elle ne devient pas fausse parce qu'une autre partie du
     * nom est illisible. On ne bascule que sur l'ABSENCE de verbe, qui est le seul cas où le résidu peut
     * cacher ce qu'on cherchait. */
    if (!verb) {
      if (!jetons.length) {
        unclassifiable.push({ name, reason: 'no A-Za-z0-9 token in the tool name — our matcher is '
          + 'Latin-only, so NO value verb could ever match it. This is not "read-only": it is unread.' });
        continue;
      }
      if (efface) {
        unclassifiable.push({ name, readable: jetons.join(' '), dropped: efface,
          reason: 'part of this tool name is outside A-Za-z0-9 and our matcher silently dropped it. No '
            + 'value verb was found in what REMAINS, but the verb may have been in what was dropped — so '
            + 'this is not "read-only", it is partially unread.' });
        continue;
      }
      readOnly.push(name);
      continue;
  ...
  return { movesValue, callerSigned, witnessesPayment, namedButNoSurface, wantsSecret, readOnly, unclassifiable };
}
...
async function vetAgent({ url, payTo, chain = 'base', knownBadScreen = null, screenFn = null,
  ...
  if (payTo) {
    ...
    /* ⚠️ SECOND APPEL RESEAU DE CE FICHIER, ET LE SEUL QUI N'ETAIT PAS BORNE — trouve le 2026-08-13.
     * Le `post()` en tete (:158) fait tout correctement: `timeout = 12000`, `on('timeout')` qui
     * DETRUIT, `resolve(null)`. Celui-ci n'avait ni l'un ni l'autre, et le fichier a servi toute la
     * nuit de MODELE canonique a copier ailleurs — parce qu'un critere « ce fichier a-t-il un timeout
     * et un handler ? » repond OUI sur un fichier qui fait DEUX requetes et n'en garde qu'UNE. Le
     * critere qui l'a trouve compte les appels et les handlers et exige handlers >= appels.
     * Le commentaire plus bas notait deja que l'echec ici est « SILENCIEUX par construction »; ce qui
     * manquait, c'est qu'il pouvait aussi ne jamais REVENIR. Mesure: 700 ms demandes, encore pendant a
     * 2501 ms sans handler, 712 ms avec `destroy()`. */
    const info = await new Promise((resolve) => {
      const req = https.get('https://base.blockscout.com/api/v2/addresses/' + payTo,
        { headers: { accept: 'application/json' }, timeout: 12000 }, (res) => {
          let d = ''; res.on('data', (c) => (d += c));
    ...
    /* ⚠️ `knownBad: false` DISAIT DEUX CHOSES OPPOSEES — corrige le 2026-07-27.
     *
     * lib/screen.js rend deja les trois etats et refuse EXPLICITEMENT de laisser lire son zero comme un
     * blanc-seing. Mesure de ses deux sorties:
     *   plancher charge, adresse propre : { blocked:false, available:true,  reason:'not on the local ... list' }
     *   plancher ABSENT                 : { blocked:false, available:false, reason:'no known-bad list loaded
     *                                       — screening UNAVAILABLE, not a clean verdict' }
     *
     * Ce module jetait cette divulgation: `!!(screened && screened.blocked)` rendait `false` dans les DEUX
     * cas, et rien dans la sortie ne disait si le crible avait seulement tourne. Un appelant lisait
     * « pas sur la liste known-bad » alors que personne n'avait regarde.
     *
     * La couche du dessous etait prudente; celle du dessus l'a aplatie. C'est la faute la plus discrete
     * de la famille: elle ne se voit pas dans le module fautif, seulement en comparant les deux.
     *
     * `screen` porte desormais QUATRE etats distincts. `knownBad` reste, inchange, pour les appelants
     * existants — il n'a jamais menti sur `true`, seulement sur ce que son `false` recouvrait. */
  ...
  /* ⛔ LA SYMETRIE QUI MANQUAIT (2026-08-11). Le crible non concluant avait sa note ci-dessus;
   * l'EXPLORATEUR non lu n'avait RIEN — alors que son echec fait tomber TROIS champs a `false` d'un
   * seul coup (`flaggedScam`, `isContract`, `verified`), et que trois `false` se lisent comme TROIS
   * FAITS quand ils sont UNE SEULE ABSENCE. `resolved` portait bien le troisieme etat, mais il etait
   * ECRIT ET LU PAR PERSONNE — le miroir du gate `rules-read-written-fields`, retourne.
   * ⚠️ Comme pour le crible, la note NE FAIT PAS basculer le verdict: un beneficiaire dont
   * l'explorateur n'a pas repondu n'est pas un mauvais beneficiaire. La divulgation remplace le refus.
   * ⚠️ Et l'echec est SILENCIEUX par construction: `https.get` ci-dessus resout `null` aussi bien sur
   * une erreur reseau que sur un JSON illisible. La note ne distingue donc pas ces deux causes — elle
   * dit seulement, honnetement, que rien n'a ete lu. */
  if (p && p.resolved === false) {
    out.unresolved = 'the payee\'s on-chain profile could NOT be read from the explorer (network error or '
      + 'unreadable response — the two are indistinguishable here). So `flaggedScam`, `isContract` and '
      + '`verified` are all false because NOTHING WAS READ, not because the address is clean, a plain EOA '
      + 'or unverified. Every verdict below is composed WITHOUT that lookup.';
  }
  ...
  /* ⚠️ UNE LISTE D'OUTILS NON LUE RENDAIT EXACTEMENT CE QUE REND UNE LISTE VIDE — mesure du 2026-07-28.
   *
   * `introspectHttp` est prudent: quand `tools/list` meurt (timeout, socket coupee, trame illisible, erreur
   * JSON-RPC), il rend `tools: null` ET la raison « answered initialize but returned no tool list ».
   * `vetAgent` aplatissait cette precaution — `out.surface = live.tools ? auditTools(...) : null`, puis la
   * phrase de verdict lisait `(s && s.toolCount) || 0`. Les deux sorties, mesurees PAR vetAgent lui-meme,
   * transport stubbe:
   *
   *   tools/list MEURT   -> verdict 'answers', « exposes 0 tool(s). None asks for key material, and none can
   *                         move value on the agent's own authority. No tool names a value-moving action at all. »
   *   tools/list rend [] -> verdict 'answers', reason IDENTIQUE OCTET POUR OCTET.
   *
   * Le degat n'est pas le zero, c'est ce qu'il autorise: TROIS AFFIRMATIONS sur des outils que personne
   * n'avait vus, sous le verdict passant. La seule trace de la panne restait dans `liveness.reason`, que
   * personne ne lit quand `verdict` et `reason` disent deja tout.
   *
   * C'est le meme aplatissement que celui corrige plus haut dans CE fichier pour le crible known-bad: la
   * couche du dessous distinguait, celle du dessus a jete la distinction. Deuxieme occurrence, meme module.
   *
   * LES DEUX BORNES. `unauditable` ne vaut QUE pour la surface NON LUE. Une liste VIDE, elle, a bien ete
   * lue: zero outil, c'est zero surface de paiement, et ca reste 'answers'. Rabattre les deux du cote
   * prudent detruirait exactement la distinction que ce correctif installe — un fail-closed qui ne
   * distingue plus n'informe plus. */
  if (url && live.tools == null) {
    return { ...out, verdict: 'unauditable',
      reason: 'the endpoint answers, but its TOOL LIST COULD NOT BE READ (' + live.reason + '). Nothing is '
        + 'claimed about what its tools can do: no tool was seen, which is not the same fact as no tool asking '
        + 'for key material and no tool moving value. Retry, or audit it with credentials — neither a pass nor a fail.',
      disclosure: DISCLOSURE };
  }

  let why = '';
  if (url) {
    const named = s.callerSigned.length + s.witnessesPayment.length + s.namedButNoSurface.length;
    /* `s.toolCount` sans garde: la porte ci-dessus garantit que la surface a ete lue sur ce chemin. Un
     * `(s && s.toolCount) || 0` defensif reinstallerait en silence le defaut mesure — si quelqu'un casse
     * la porte, mieux vaut que ca jette que ca affirme. */
    why = 'the endpoint answers and exposes ' + s.toolCount + ' tool(s). None asks for key ' +
