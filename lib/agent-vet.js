...
function auditTools(tools) {
  ...
  for (const t of tools || []) {
    ...
    /* Un nom ILLISIBLE par notre tokenizer ne peut pas être classé lecture seule: on n'a pas cherché de
     * verbe dedans, on n'a rien pu y chercher du tout. La distinction est faite AVANT `readOnly`, sinon
     * l'absence de verbe se lit comme l'absence de danger. */
    if (!jetons.length) {
      ...
      /* ⚠️ LIMITE CONNUE, MESURÉE, NON RÉSOLUE ICI: le nom PARTIELLEMENT illisible.
       * 送金Amount tokenise en ['amount'] — liste non vide, donc il ne passe pas par ici et retombe
       * dans readOnly. Or c'est le VERBE qui a été effacé (送金 = envoyer de l'argent) et le CHAMP qui
       * a survécu: readOnly reste une sur-affirmation pour lui, juste plus discrète. Le traiter
       * demande de comparer le nom d'origine à ce que le tokenizer en a gardé. À faire; d'ici là
       * cette limite est ÉCRITE plutôt que masquée — un correctif qui couvre 2 cas sur 3 en laissant
       * croire qu'il les couvre tous est pire que pas de correctif. */
      continue;
    }
