 ...
 */
function observationThin({ status, siblingCount, siblingCountCensored, gradedForward } = {}) {
  const a = annonce();
  if (!a) return null;
  ...
  /* ⛔ UN PLANCHER ETABLIT « AU MOINS N », JAMAIS « MOINS DE N ». `token-radar.js:660` pose
   * `siblingCountCensored` quand l'historique du financeur n'a pas tenu dans le scan: `siblingCount`
   * n'est alors qu'une BORNE INFERIEURE. Or la branche « sure » de cette lecture est definie par
   * `siblingCount < SEUIL` — une borne SUPERIEURE, que le plancher ne peut pas etablir.
   *
   * ⚠️ ET L'ASYMETRIE EST LE COEUR DE LA CHOSE: un plancher de 50 prouve bien `>= 20`, donc le cote
   * DANGER n'est pas touche. Seul le cote SUR reposait sur une borne absente.
   *
   * Mesure du 2026-08-10 sur la branche `< 20`: 412 tokens, dont 149 planchers et 33 sans drapeau —
   * soit 44,2 pct dont la borne n'est PAS etablie. Et les deux populations n'ont rien a voir: 13,5 pct
   * de rugs cote planchers contre 76,8 pct cote comptes prouves. Les melanger fabriquait une branche
   * « sure » a 50,6 pct qui, nettoyee, remonte a 76,8 — contre une base de 83,9.
   *
   * ⚠️ TROIS ETATS: seul un `false` EXPLICITE etablit la borne. `true` est un plancher, `null` veut dire
   * que le radar n'a rien ecrit — et « on ne sait pas » n'est pas « etabli ». */
  if (siblingCount < SEUIL_FRERES && siblingCountCensored !== false) {
    return { ...base, boundChecked: false, applies: null, rate: null, siblingCount,
      siblingCountCensored: siblingCountCensored === true ? true : null,
      why: 'the sibling count is a FLOOR, not a count: the funder history did not fit inside the scan '
        + '(or the node never recorded whether it did). A floor establishes "at least ' + siblingCount
        + '", never "fewer than ' + SEUIL_FRERES + '" — so the condition this reading depends on is NOT '
        + 'established, and the rate is WITHHELD rather than guessed.',
      howToCheck: 'raise the funder-trace page budget until the scan completes, then call again' };
  }
