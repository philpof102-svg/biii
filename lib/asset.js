...
function assessAsset({ token, claimedIssuer, claimedSymbol } = {}, { registry = SEED_REGISTRY, denylist } = {}) {
  ...
  const deny = denylist instanceof Set ? denylist : new Set([...(denylist || [])].map(lower));
  ...
  if (entry) {
    ...
    if (claimedIssuer && norm(claimedIssuer) !== norm(entry.issuer))
  ...
  if (claimedSymbol || claimedIssuer) {
    ...
    if (known && lower(known.address) !== addr)
      return { status: 'impersonation',
        reason: `claims ${claimedSymbol || claimedIssuer} but the genuine ${known.issuer} ${known.symbol} is ${known.address}, not this contract`,
