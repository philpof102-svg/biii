...
/* `symbols` est une ILLUSTRATION plafonnee, pas un inventaire. `priorFunded` compte des TOKENS et ne
 * peut donc pas servir de total: un financeur peut lancer trente tokens sous cinq symboles. Sans
 * `symbolsTotal`, un appelant qui compte la liste croit avoir tout vu des qu'elle tient sous le
 * plafond — et ne peut pas distinguer « douze symboles » de « douze montres sur quarante ». */
const SYM_CAP = 12;
...
function assess(registry, funder, opts = {}) {
  if (!funder) {
    return {
       ...
       * que cette phrase-ci soit revee.
       *
       * ⚠️ CE COMMENTAIRE PORTAIT LUI-MEME UN CHIFFRE PERIME — corrige le 2026-07-30 au soir.
       * J'y avais ecrit « 34 % ont ECHOUE » sur 32 traces. Avec 217 traces et le classement affine du
       * meme jour, la decomposition reelle est:
       *     113  tracees
       *      91  l'explorateur A REPONDU, aucun createur indexe   <- un CONSTAT, pas une panne
       *       8  l'explorateur n'a pas repondu                    <- la seule vraie panne
       *       5  tracees, aucun financeur trouve
       * Soit ~4 % d'echec reel, pas 34. Les 91 n'etaient pas reessayables et gonflaient le taux.
       * J'allais chasser un rate-limit qui n'existe pas — c'est le split `tokenRead` pose le matin meme
       * qui l'a dit. Un chiffre ecrit dans un commentaire vieillit exactement comme un chiffre publie:
       * celui-ci a tenu neuf heures.
  ...
  if (g.rugged > 0) {
    return {
      ...
      priorRugs: g.rugged, priorFunded: g.funded,
      symbols: [...g.symbols].slice(0, SYM_CAP), symbolsTotal: g.symbols.size,
    };
  }
  return {
    ...
    priorRugs: 0, priorFunded: g.funded,
    symbols: [...g.symbols].slice(0, SYM_CAP), symbolsTotal: g.symbols.size,
  };
}
