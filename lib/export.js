...
const { summary, receiptNo, windowRows, blockTimeOf, undatedNote,
  microStrict, amountMicroOf, tipMicroOf, unreadableAmountNote } = require('./ledger');
...
// settlement date from the on-chain block time (unix seconds) → YYYY-MM-DD; empty if unknown. Reads the
// timestamp through ledger.blockTimeOf — THE single definition of "is this receipt dated" (see ledger.js).
function isoDate(blockTime) {
  const t = blockTimeOf({ blockTime });
...
/* La fenetre passe par ledger.windowRows — LA regle unique. Un recu sans date sort d'une fenetre datee et
 * EST COMPTE dans `undatedExcluded`, que buildExport publie: un CSV comptable qui omet une ligne sans le
 * dire est pire qu'un CSV qui refuse, parce qu'il a l'air complet devant le fisc. */
function windowed(rows, window) {
  return windowRows(rows, window);
}

/** One accounting line (as an object) from a verified receipt + its ledger number. */
function rowFor(no, r) {
  /* ⚠️ `String(r.paidMicro || r.amountMicro || '0')` ecrivait `0.00` dans une colonne comptable pour un
   * montant ABSENT — indistinguable d'une vente reelle a zero, et importe tel quel dans QuickBooks. Un
   * CSV qui remplit une case qu'il n'a pas su lire est pire qu'une case qui crie: le comptable ne relira
   * jamais la ligne. `UNREADABLE` fait echouer l'import de CETTE ligne, ce qui est la bonne issue. */
  const brut = amountMicroOf(r), pourboire = tipMicroOf(r), facture = microStrict(r.amountMicro);
  const usd = (v) => (v === null ? 'UNREADABLE' : T.microToUsd(v.toString()));
  return {
    ...
    payer: r.payer || '',
    gross_usdc: usd(brut),
    tip_usdc: usd(pourboire),
    charged_usdc: usd(facture === null ? brut : facture),
    token: r.token || 'USDC',
...
function toCsv(rows, { window } = {}) {
  const { kept } = windowed(normalizeRows(rows), window);
...
function buildExport(rows, { window, brand } = {}) {
  ...
  /* ⚠️ CETTE PROJECTION EST L'ENDROIT OU LA CORRECTION SERAIT MORTE. `summary` ne descend pas ici par
   * spread: ses champs sont recopies un par un, donc un `undatedExcluded` ajoute a ledger.js et oublie sur
   * cette ligne serait produit correctement et n'atteindrait AUCUN appelant. C'est exactement le mode
   * d'echec du 2026-08-04 (trois corrections mortes a la couche de mise en forme). Il est donc recopie
   * explicitement, et test/undated-receipts-disclosed.test.js verifie qu'il arrive jusqu'au bout. */
  return {
    format: 'csv',
    columns: COLUMNS,
    csv: toCsv(rows, { window }),
    summary: { count: s.count, grossUsd: s.grossUsd, tipsUsd: s.tipsUsd, txHashes: s.txHashes,
      undatedExcluded: s.undatedExcluded, unreadableAmounts: s.unreadableAmounts },
    filename: (label ? String(label).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') + '-' : '') + 'biii-books.csv',
    // La divulgation en TETE quand elle existe: un comptable lit le premier mot, pas le dernier.
    disclosure: (s.undatedExcluded ? undatedNote(s.undatedExcluded, 'en') + ' ' : '')
      + (s.unreadableAmounts ? unreadableAmountNote(s.unreadableAmounts, 'en') + ' ' : '')
      + 'ACCOUNTANT-READY export. Every row carries its txHash + Basescan link — re-verify each '
...
module.exports = { toCsv, buildExport, csvField, COLUMNS, normalizeRows, windowed };
