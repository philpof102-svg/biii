...
/* Bounded 2026-08-13, same measurement as feeder.js and approvals.js: a host that accepts and never
 * answers does not trigger `on('error')`, and a resolve-only promise then hangs forever. 12s and the
 * destroying handler come from agent-vet.js:158. resolve(null) is the failure this module already
 * models, so a hang becomes an answer its callers already handle. */
...
async function assessRecoveryOffer({ address, chain = 'base', asksForSignature, asksForUpfrontPayment,
  ...
  /* ⚠️ « THE CHAIN SHOWS NOTHING CONCLUSIVE » SUR UNE CHAINE JAMAIS LUE.
   ...
   *
   * Dans deux cas sur trois aucune lecture n'a eu lieu, et la phrase affirme le contraire. La detection
   * de fraude par les DEMANDES (seed, signature, paiement d'avance, installation) est intacte — c'est le
   * coeur de cet outil et il tient. Ce qui est faux, c'est la moitie qui parle de la chaine.
   *
   * Et une couche plus bas, le meme motif: `((inTx && inTx.items) || [])` faisait d'un fetch echoue zero
   * expediteur, donc `readsAsHarvesting: false` et « Nothing conclusive in the flow » — sur un flux qu'on
   * n'a pas ouvert. Une adresse malformee partait aussi vers l'explorateur.
   *
   * Quatre etats desormais: pas tente / adresse invalide / illisible / lu. */
  let chainRead = 'not_attempted';
  let chainEvidence = null;
  const api0 = EXPLORER[String(chain).toLowerCase()];
  if (address && api0 && !/^0x[0-9a-f]{40}$/.test(String(address).toLowerCase())) chainRead = 'invalid_address';
  else if (address && api0) {
    ...
    /* `getJSON` resout `null` sur panne ET sur corps non-parsable. Si une des trois lectures manque, le
     * flux n'a pas ete vu — et un flux non vu ne peut porter aucune conclusion, ni a charge ni a
     * decharge. On le dit au lieu de compter zero. */
    /* ⚠️ `inTx && outTx && info` ne verifiait que la PRESENCE d'un corps, pas sa FORME. Un corps rendu
     * sans tableau `items` — derive de schema, page d'erreur en JSON, reponse tronquee — passait donc
     * pour « read », et les `|| []` juste en dessous produisaient `distinctSenders: 0, inboundCount: 0`
     * comme des FAITS MESURES. La phrase publiee reste couverte (« absence of a pattern is NOT evidence
     * of legitimacy »), mais `chainEvidence` est la partie que lit une machine, et elle affirmait un
     * comptage qui n'a jamais eu lieu. Meme angle mort ferme le meme jour dans lib/feeder.js. */
    const bienForme = (x) => x && Array.isArray(x.items);
    chainRead = (bienForme(inTx) && bienForme(outTx) && info) ? 'read' : 'unreadable';
    ...
    chainEvidence = {
      ...
      note: senders.size >= MANY_SENDERS && oneWay
        ...
        : chainRead === 'unreadable'
          ? 'The flow could NOT be read (the explorer did not answer), so the counts above are not a measurement — they are what an unread page looks like. Nothing here is evidence either way.'
  ...
  const reason = fatal.length
    ...
    : (chainEvidence && chainEvidence.readsAsHarvesting)
      ? chainEvidence.note
      /* La phrase ne parle de la chaine QUE si la chaine a ete lue. Le reste — « ceci n'est pas un
       * blanc-seing » — vaut dans les quatre cas et ne bouge pas. */
      : 'No fatal request was reported, and ' + ({
        read: 'the chain shows nothing conclusive',
        unreadable: 'THE CHAIN COULD NOT BE READ (the explorer did not answer) — that is not "nothing found"',
        invalid_address: 'NO CHAIN CHECK WAS POSSIBLE: the address given is not a 0x address (40 hex), so nothing was looked up',
        not_attempted: 'NO CHAIN CHECK WAS RUN (no address, or this chain is not wired here) — nothing was looked up',
      }[chainRead]) + '. This is NOT a clearance: if at any point they ask you to sign, pay first, '
      + 'install something, or share a key, the answer is already no.';

  /* L'etat de la lecture voyage avec le verdict: un appelant doit pouvoir distinguer « regarde, rien
   * trouve » de « pas regarde », sans relire cette phrase au mot pres. */
