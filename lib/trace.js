 ...
 */
/**
 * Une adresse TRON, prete a entrer dans un CHEMIN d'URL — ou `null` si ce n'en est pas une.
 *
 * ⛔ `current` allait BRUT dans `TRONGRID + '/v1/accounts/' + current`. L'appelant MCP
 * (`till_trace_theft`) ne verifiait que la PRESENCE du champ, jamais sa forme, et l'argument vient
 * d'un LLM. Mesure du 2026-08-15, hors ligne via `lireJson`:
 *
 ...
 *
 * 🔴 Les deux derniers sont les pires, et pas pour la raison evidente: le segment `/transactions`
 ...
 * la boucle s'arrete sans sortie, et l'adresse est rapportee comme TERMINUS — c'est-a-dire comme la
 * destination des fonds. Ce module dit lui-meme qu'un faux terminus est « le defaut le plus couteux
 * du module », et il a servi sur un vol reel.
 *
 * Une adresse TRON est du base58 (pas de 0, O, I, l), 34 caracteres, commencant par T: tout est deja
 * sur-safe pour une URL, donc valider ne change RIEN pour une entree legitime — et refuse le reste
 * avant le premier appel reseau. `classifyB20` fait exactement ca depuis toujours; c'est la
 * convention de la maison, pas une invention.
 */
const TRON_BASE58 = /^T[1-9A-HJ-NP-Za-km-z]{33}$/;
function segmentTron(v) {
  const s = String(v == null ? '' : v);
  return TRON_BASE58.test(s) ? encodeURIComponent(s) : null;
}
...
async function followTron(address, { maxHops = 3, lireJson } = {}) {
  ...
  const hops = [];
  let current = address;
  if (!segmentTron(current)) {
    /* Refus AVANT tout appel: on ne part pas en trace sur quelque chose dont on ne sait pas que
     * c'est une adresse. Meme forme de retour que les autres arrets, pour qu'aucun lecteur ne
     * confonde « refuse » avec « rien trouve ». */
    /* MEME FORME que les autres retours de cette fonction — `ok`, `hops`, `stoppedBecause`,
     * `complete`, `stopNote`, `note`. Un refus qui invente ses propres noms de champs est invisible
     * pour un appelant qui lit ceux-la; c'est le meme motif que la divergence entre `bridge` et
     * `tron` documentee dans test/trace-theft-error-shape.test.js. */
    return { ok: false, hops: [], stoppedBecause: 'not_an_address', complete: false,
      reason: 'not a well-formed TRON address',
      stopNote: 'refused BEFORE any lookup: this is not a well-formed TRON address (base58, 34 chars, '
        + 'starting with T). Nothing was requested, and nothing is claimed about this value.',
      note: 'STRUCTURE ONLY. Nothing was read, so nothing here is evidence of anything.' };
  }
  let arret = null;                 // POURQUOI la piste s'arrete — jamais laisse implicite
  for (let hop = 0; hop < maxHops && current; hop++) {
    /* Le saut suivant vient d'une REPONSE d'API (`hexToTron`), pas de l'appelant: il se revalide donc
     * a chaque tour, sinon une reponse hostile piloterait les URLs des sauts suivants. */
    const seg = segmentTron(current);
    if (!seg) { arret = 'hop_not_an_address'; break; }
    const acct = await lire(TRONGRID + '/v1/accounts/' + seg);
    ...
    const txs = await lire(TRONGRID + '/v1/accounts/' + seg + '/transactions?limit=40');
    ...
    /* ⛔⛔ « PAS DE SUITE » N'EST PAS « PAS DE SORTIE ». `hexToTron` rend `null` sur un
     * `to_address` qu'il ne sait pas convertir (mesure du 2026-08-15). L'ancienne ligne ecrasait
     * alors `current` a null et posait `arret = 'no_outbound'` — or c'est la SEULE raison que cette
     * fonction traite comme une trace COMPLETE (`complete: arret === 'no_outbound'`), et dont le
     * texte dit « the last address has no outbound transfer ». Une sortie EXISTAIT: seule sa
     * destination etait illisible. On rapportait donc un TERMINUS fabrique, dans un module dont le
     * commentaire d'a cote dit deja que le faux terminus est son defaut le plus couteux — la meme
     * lecon, une ligne plus bas, sur une autre cause. */
    const suivant = passThrough ? passThrough.to : (outs[0] ? outs[0].to : null);
    if (!suivant) { arret = outs.length ? 'next_hop_unreadable' : 'no_outbound'; break; }
    current = suivant;
  }
  ...
  const RAISONS = {
    /* Un saut refuse n'est ni un terminus ni une non-lecture: la piste s'arrete parce que ce que la
     * reponse precedente a nomme n'est pas une adresse TRON. Le dire, sinon la derniere adresse LUE
     * passerait pour la destination. */
    next_hop_unreadable: 'an outbound transfer WAS found, but its destination could not be decoded into '
      + 'a TRON address. The trail stops here for lack of a next hop — this address is NOT a terminus, '
      + 'and the funds did leave it.',
    hop_not_an_address: 'the trail stopped because the next hop named by the previous response is not a '
      + 'well-formed TRON address. The last address here is a WAYPOINT, not a destination, and nothing '
      + 'was requested for the malformed value.',
