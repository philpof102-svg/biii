...
function scanText(text, index, { minRun = 12 } = {}) {
  ...
  let run = [];
  const flush = () => {
    // Math.max(1, …): un `minRun: 0` passe par un appelant faisait tomber judgeRun sur un run vide.
    if (run.length >= Math.max(1, minRun)) findings.push(judgeRun(run, index));
    run = [];
  };
...
function judgeRun(run, index) {
  // Un run vide n'a pas de position, donc pas de constat a rendre. Sans ce garde on lisait `run[0].line`
  // et on jetait un TypeError anonyme; l'export est appelable de l'exterieur, il dit maintenant pourquoi.
  ...
  const spanOf = (a, b) => (b > a ? ' spanning lines ' + a + '-' + b : '');
  ...
  if (best) {
    /* ═══ LA POSITION RENDUE EST CELLE DE LA PHRASE, PLUS CELLE DU RUN QUI LA CONTIENT ═══
     * Ce module ne publie JAMAIS la phrase — la position EST sa seule sortie. Elle etait celle du run:
     * mesure du 2026-08-14, meme vecteur BIP-39 valide de 12 mots, liste verticale numerotee:
     *
     *   3 mots parasites avant   -> phrase lignes 4-15,  rendu « words: 12, spanning lines 1-15 »
     *   20 mots de liste apres   -> phrase lignes 1-12,  rendu « words: 12, spanning lines 1-32 »
     *   DEUX phrases collees     -> phrases 1-12 ET 13-24, rendu UN SEUL constat « 12, lines 1-24 »
     *
     * Les deux premiers designent des lignes qui ne portent pas la phrase (« at this location » sur la
     * ligne 1 alors qu'elle commence a la 4). Le troisieme est le vrai defaut: seule la PREMIERE fenetre
     * valide d'un run est rendue, et l'intervalle gonfle RECOUVRAIT la seconde — l'omission devenait donc
     * invisible, un « words: 12 » a cote d'un intervalle de 24 lignes.
     *
     * Retrecir en silence aurait echange un angle mort contre un autre: le lecteur qui voit lignes 4-15 ne
     * saurait plus qu'il y a d'autres mots de la liste autour, la ou justement une seconde phrase peut se
     * tenir. La borne voyage donc AVEC le chiffre — `runLine`/`runEndLine`/`runWords` sont toujours rendus,
     * et le run plus large que la phrase se DIT dans la note. */
    const startTok = run[best.start];
    const endTok = run[best.start + best.len - 1];
    const line = startTok.line, endLine = endTok.line;
    const larger = run.length > best.len;
    return { line, endLine, words: best.len, runLine, runEndLine, runWords: words.length, verdict: 'confirmed',
      note: 'A valid BIP-39 checksum' + spanOf(line, endLine) + '. This is a recovery phrase, in cleartext, at '
        + 'this location.' + (larger
          ? ' It sits inside an unbroken run of ' + words.length + ' BIP-39 words' + spanOf(runLine, runEndLine)
            + '. Only the FIRST valid window in a run is reported, so a second phrase inside that run would '
            + 'not appear here — read the whole run, not just the lines named above.'
          : '') };
  }
  return { line: runLine, endLine: runEndLine, words: words.length,
    runLine, runEndLine, runWords: words.length, verdict: 'wordlist_run',
    note: words.length + ' consecutive BIP-39 words' + spanOf(runLine, runEndLine) + ' with no valid checksum at the start of the run. ' +
...
function scanPaths(paths, index, { maxFiles = 20000 } = {}) {
  ...
  const findings = [];
  /* `absent` est SEPARE d'`unreadable`: un dossier qui n'est pas sur cette machine (pas de OneDrive, pas
   * de ~/Notes) n'a rien a lire, alors qu'un dossier qui existe et refuse de s'ouvrir est une couverture
   * perdue. Les confondre faisait compter 2 « illisibles » sur les 6 chemins par defaut de CETTE machine,
   * avant meme le premier fichier — mesure du 2026-07-28. */
  ...
  /* ═══ LA PORTEE VOULUE N'EST PAS UNE COUVERTURE PERDUE ═══
   * `complete` valait `totalSkipped === 0`, ce qui additionnait deux choses opposees: les images qu'on ne
   * SAIT PAS lire (exclusion voulue, connue d'avance) et ce qui a BLOQUE une lecture qu'on comptait faire.
   * Mesure du 2026-07-28, deux jeux opposes:
   *
   *   dossier propre + 2 images            -> complete: false
   *   le meme + un chemin qui n'existe pas -> complete: false      (IDENTIQUE)
   *
   * Le drapeau ne distinguait donc rien — et comme tout dossier Documents reel contient des images, il
   * valait false 100 % du temps. Un chiffre qui ne change jamais n'observe pas, il affirme; c'est le
   * critere que ce module applique lui-meme ailleurs. Sa propre divulgation dit pourtant « Check `skipped`
   * and `complete` », en renvoyant le lecteur vers un champ sans information.
   *
   * `complete` porte desormais UNIQUEMENT les lectures empechees. `notTextual` reste compte et publie —
   * c'est une limite reelle du scan — mais dans `scope`, pas dans le verdict de couverture. Et `absent`
   * n'est ni l'un ni l'autre: il n'y avait rien a lire la.
   *
   * Personne ne branchait sur ce champ (seul appelant: le handler MCP, qui rend l'objet entier), donc la
   * redefinition ne casse aucun consommateur — verifie avant de la faire. */
  ...
  return {
    ...
    complete: bloquees === 0,
    coverage: bloquees === 0
      ? (skipped.notTextual || skipped.absent
        ...
        : 'as_intended — nothing blocked a read, and every path given was present and textual.')
    ...
    disclosure: 'This never says "safe". A confirmed hit is a FACT — the BIP-39 checksum validated — and the ' +
      ...
      + 'whether anything BLOCKED a read (complete:false) or whether the only gaps are the ones this scan '
      + 'never claimed to cover. `skipped` carries the counts behind it.',
  };
}
