...
function readingFor(address, opts = {}) {
  ...
  return {
    ...
    /* ⛔ `siblingCount` EST UN PLANCHER, PAS UN COMPTE, QUAND LE SCAN N'A PAS FINI — `token-radar.js:660`
     * le dit et pose `siblingCountCensored` (ligne 674). Ce drapeau ne circulait NULLE PART: ni ici, ni
     * dans `meme.js`, ni dans `thin-risk.js`. Or un PLANCHER etablit « au moins N » et JAMAIS « moins de
     * N » — donc toute regle dont la condition est `siblingCount < seuil` reposait, pour la part
     * censuree, sur une borne qu'elle n'avait pas.
     *
     * Mesure du 2026-08-10 sur la branche « sure » du pari `thin` (`< 20`): 412 tokens, dont 149
     * PLANCHERS (borne non etablie), 230 vrais comptes, 33 sans drapeau. Et les deux populations n'ont
     * rien a voir: 13,5 % de rugs cote planchers contre 76,8 % cote comptes prouves.
     *
     * ⚠️ TROIS ETATS, pas deux: `true` (plancher), `false` (compte etabli), `null` (le radar n'a rien
     * ecrit — on ne sait pas, et ce n'est pas « etabli »). */
    siblingCountCensored: typeof t.siblingCountCensored === 'boolean' ? t.siblingCountCensored : null,
