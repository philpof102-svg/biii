...
function readingFor(address, opts = {}) {
  if (typeof address !== 'string' || !/^0x[0-9a-fA-F]{40}$/.test(address)) {
    return { state: 'miss', siblingCount: null, symbolVerdict: null, observedAt: null,
      detail: 'ce n est pas une adresse 0x de 40 hex — rien n a ete cherche' };
  }
  ...
  if (!loaded) {
    return { state: 'no_database', siblingCount: null, symbolVerdict: null, observedAt: null,
      detail: 'aucune base d observation sur ce noeud — etat NORMAL d un noeud neuf, pas une panne' };
  }
  if (loaded.unreadable) {
    return { state: 'unreadable', siblingCount: null, symbolVerdict: null, observedAt: null,
      detail: 'la base existe mais ne se lit pas (' + loaded.detail + ') — ce n est PAS un demarrage a froid' };
  }
  const t = loaded.byToken.get(address.toLowerCase());
  if (!t) {
    return { state: 'miss', siblingCount: null, symbolVerdict: null, observedAt: null,
      detail: 'ce token n a jamais ete observe par ce noeud — son financeur reste a tracer' };
  }
  return {
    state: 'hit',
