...
function scoreCalls(all, now) {
  ...
  /* THE MATURITY WINDOW IS DERIVED, NOT CHOSEN.
   *
   * It is the slowest rug ever observed, rounded up. A flagged token older than that has outlived every rug
   * in the dataset, which is what makes it a false alarm rather than a pending one.
   *
   * Choosing the window by hand would repeat the error that killed two escalation rules in this project: a
   * threshold picked while looking at the outcomes it will be scored against has proven nothing, however
   * good the resulting number looks. Deriving it means the window moves when the market does, and it moves
   * against us as often as for us. */
  ...
  const maturityH = slowestRugH == null ? null : Math.ceil(slowestRugH);
  ...
  return {
    ...
    maturityWindowBasis: slowestRugH == null
      ? 'no rug observed yet — nothing can be resolved as a false alarm, so every live flag stays open'
      : 'the slowest rug ever observed took ' + slowestRugH.toFixed(1) + 'h; a flagged token older than that '
        + 'has outlived every rug in this dataset, which is what makes it a false alarm rather than a pending one',
