 ...
 *
 * @returns { maturityH, p95RugH, slowestRugH, beyondWindow, lifetimes } — nulls when no rug is observed
 *          yet, which is a real state and not a zero.
 */
function maturityWindow(rows) {
  const lifetimes = rows
    .filter((t) => t.outcome === 'rugged')
  ...
  return { maturityH, p95RugH, slowestRugH, beyondWindow, lifetimes };
}
...
function scoreCalls(all, now, opts = {}) {
  ...
  const { maturityH, slowestRugH, beyondWindow, lifetimes: ruggedLifetimesH } = maturityWindow(rows);
