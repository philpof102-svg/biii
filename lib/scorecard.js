...
/**
 * The maturity window, EXPORTED rather than reimplemented downstream.
 *
 * The reasoning behind p95-rather-than-maximum is long and lives at its only call site below. It is
 * exported because a second consumer appeared — the prequential harness needs the same window to decide
 * whether a token's outcome was already KNOWN at the moment another token was being judged — and this
 * repository's most repeated defect is a correct helper sitting one file away from a weaker copy of
 * itself. Two windows that agree today would drift apart the first time either is tuned, and the
 * divergence would be invisible: both sides would still print a number.
 *
 * ⛔ UN RUG DONT ON NE SAIT PAS LIRE LA DUREE DE VIE N'EST PAS UN RUG DE MOINS.
 * La chaine de `.filter()` jetait en silence toute ligne `rugged` dont `ruggedAt`/`firstSeen` ne se
 * parsait pas (ou dont l'ecart etait negatif — une corruption, pas une duree). Sortie IDENTIQUE, octet
 * pour octet, a celle d'un jeu de donnees ou ces rugs n'existent pas. Mesure du 2026-08-07, memes
 * lignes, seules les dates rendues illisibles :
 *
 *     dates lisibles   -> maturityWindowHours 3   strongCallsWrong 3   strongCallsOpen 0
 *     dates illisibles -> maturityWindowHours null strongCallsWrong 0   strongCallsOpen 3
 *
 * Les MEMES trois faux positifs disparaissent du compte de nos erreurs. C'est l'auto-flatterie que le
 * long commentaire du p95 ci-dessous dit refuser — arrivee par la porte de derriere, celle de la
 * donnee illisible au lieu de celle de la statistique choisie.
 *
 * @returns { maturityH, p95RugH, slowestRugH, beyondWindow, lifetimes, ruggedRows, unreadable }
 *          — nulls quand aucun rug n'est observe, ce qui est un etat reel et pas un zero ; `unreadable`
 *          distingue le troisieme etat, « des rugs observes dont la duree de vie n'a pas pu etre lue ».
 */
function maturityWindow(rows) {
  const ruggedRows = rows.filter((t) => t.outcome === 'rugged');
  const lifetimes = ruggedRows
    .map((t) => (Date.parse(t.ruggedAt) - Date.parse(t.firstSeen)) / HOURS)
    .filter((h) => Number.isFinite(h) && h >= 0)
    .sort((a, b) => a - b);
  const unreadable = ruggedRows.length - lifetimes.length;
  const p95RugH = quantile(lifetimes, 0.95);
  const slowestRugH = lifetimes.length ? lifetimes[lifetimes.length - 1] : null;
  const maturityH = p95RugH == null ? null : Math.max(1, Math.ceil(p95RugH));
  const beyondWindow = maturityH == null ? 0 : lifetimes.filter((h) => h > maturityH).length;
  return { maturityH, p95RugH, slowestRugH, beyondWindow, lifetimes, ruggedRows: ruggedRows.length, unreadable };
}

/* When `basisAtFirstSight` went live, so a row that predates it is not a breach.
 *
 * Not estimated — MEASURED against the database, and the boundary is clean with no overlap: the last row
 * without a basis was seen at 20:13:35Z and the first row with one at 20:41:55Z. Guessing a round hour
 * either way would have made 64 legitimate rows look broken or hidden the first real breach. */
const BASIS_SINCE = Date.parse('2026-07-26T20:41:55.133Z');
...
function scoreCalls(all, now, opts = {}) {
  ...
  const { maturityH, slowestRugH, beyondWindow, lifetimes: ruggedLifetimesH,
    unreadable: unreadableLifetimes } = maturityWindow(rows);
  ...
  return {
    ...
    /* LE DENOMINATEUR DE LA FENETRE N'EST PAS `rugsObserved`, ET IL FAUT LE DIRE.
     * La phrase disait « N of M observed rugs » avec M = le nombre de durees de vie LISIBLES, pendant
     * que le meme objet publiait `rugsObserved` = toutes. Deux denominateurs sous un seul mot. */
    rugsWithReadableLifetime: ruggedLifetimesH.length,
    rugsWithUnreadableLifetime: unreadableLifetimes,

    /* Trois branches, pas deux. « aucun rug observe » et « des rugs observes, aucun lisible » tombaient
     * sur la MEME phrase — la rassurante — alors que la seconde est un trou de donnees qui EFFACE nos
     * faux positifs (mesure dans l'en-tete de `maturityWindow`). Le `else` final qui rassure est
     * exactement le motif que ce depot retire partout ailleurs. */
    maturityWindowBasis: maturityH == null
      ? (rugged.length
        ? rugged.length + ' rug(s) observed, but NOT ONE has a readable lifetime (unparseable or '
          + 'inconsistent firstSeen/ruggedAt), so the window could not be derived at all. Nothing resolves '
          + 'as a false alarm and every live flag stays open — that is a GAP IN THE DATA, not a clean '
          + 'record. Read strongCallsWrong as unmeasured here, not as zero.'
      ...
      : '95th percentile of observed rug lifetimes, rounded up (' + maturityH + 'h; slowest ever seen '
        ...
        + 'with no call improving. ' + beyondWindow + ' of ' + ruggedLifetimesH.length + ' rugs WITH A READABLE '
        + 'LIFETIME landed beyond this window, so roughly that share of what we count against ourselves are '
        + 'slow catches. That is the deliberate side to be wrong on.'
        + (unreadableLifetimes
          ? ' ⚠️ ' + unreadableLifetimes + ' of the ' + rugged.length + ' observed rugs carry NO readable '
            + 'lifetime and took no part in deriving this window; they are excluded from the denominator '
            + 'above rather than counted as fast.'
          : ''),
    ...
    /* COVERAGE BREACH — a verdict that reached the ledger without recording what it was based on.
     *
     * Borrowed from block/buzz's conformance crate, which states it better than we had: an entry into a
     * critical seam that emits NO trace records an implementation bug and fails closed. Without that, a
     * conformance gate only proves things about the paths that bothered to report.
     *
     * Our seam is the verdict. `basisAtFirstSight` was added 2026-07-27 so a later claim of "we knew because
     * X" is a lookup rather than an argument — but nothing FAILED when a verdict skipped it, so a new code
     * path could quietly stop recording and every number here would stay green. This counts them.
     *
     * Rows older than the feature legitimately have none and are excluded by date; they are reported
     * separately as `preBasisRows` so the denominator cannot hide either. */
    verdictsWithoutBasis: rows.filter((t) => Date.parse(t.firstSeen) >= BASIS_SINCE && !t.basisAtFirstSight).length,
    preBasisRows: rows.filter((t) => Date.parse(t.firstSeen) < BASIS_SINCE).length,
...
module.exports = { scoreCalls, isStrongCall, maturityWindow };
