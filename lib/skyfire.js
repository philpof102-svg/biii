...
function kyaLens(token, { verified = false, expectedAudience = null, requireExpiry = false, now = Date.now() } = {}) {
  ...
  /* ⚠️ `Number(p.exp) || 0` ECRASAIT UNE EXPIRATION ILLISIBLE SUR ZERO — c'est-a-dire sur la valeur qui
   * signifie « pas d'expiration ». Mesure du 2026-07-28, meme jeton par ailleurs:
   *
   ...
   *   exp = '2020-01-01…' -> idem
   *
   * La lentille publiait donc « ⚠ NO EXPIRY: this vouch does not expire » pour un jeton qui EN PORTE
   * une: une propriete deduite d'un parse rate. C'est la troisieme occurrence de la meme classe dans ce
   * fichier, qui dit pourtant lui-meme « absence of a valid token is never identity ».
   *
   * Trois etats, pas deux: pas de champ · champ lisible · champ present et illisible. */
  const expBrut = p.exp;
  const expPresent = expBrut != null && expBrut !== '';
  const expNum = expPresent ? Number(expBrut) : 0;
  const expLisible = expPresent && Number.isFinite(expNum) && expNum > 0;
  const expIllisible = expPresent && !expLisible;
  const expSec = expLisible ? expNum : 0;
  ...
  /* `expires` porte desormais TROIS etats: true (lisible), false (aucun champ), null (present mais
   * illisible). `null` n'est pas `false`: on ne sait pas, et on ne le deduit pas d'un echec de lecture. */
  const posture = { expires: expIllisible ? null : expSec > 0, audienceBound: expectedAudience != null };
  const postureNote = (posture.expires === true ? ''
    : posture.expires === null
      ? ' ⚠ EXPIRY UNREADABLE: this token CARRIES an exp claim (' + JSON.stringify(String(expBrut)).slice(0, 40)
        + ') that is not a readable unix epoch. It is NOT "no expiry" — we could not read it, so the vouch '
        + 'may well be expired. Fix the issuer, or reject with requireExpiry.'
      : ' ⚠ NO EXPIRY: this vouch does not expire — treat a leaked token as valid indefinitely.')
...
function authorizeCharge(spendAuth, charge, { spentMicro = 0, now = Date.now(), verified = false } = {}) {
  ...
  /* ⚠️ UNE EXPIRATION ILLISIBLE VALAIT « PAS D'EXPIRATION ». L'ancienne ligne etait
   *   `a.exp != null && Number(a.exp) > 0 && now > Number(a.exp) * 1000`
   * et `Number('abc')` vaut NaN, donc `NaN > 0` est faux: la condition se court-circuitait et AUCUN refus
   * n'etait emis. Mesure du 2026-07-28, charge USDC/Base valide:
   *
   ...
   *   exp = ''                        -> AUTORISE
   *
   * C'est le drain exact que l'expiration existe pour empecher: une autorisation signee dont la date est
   * ecrite en ISO au lieu d'epoque — l'erreur d'integration la plus banale — ne finit jamais.
   *
   * La regle etait DEJA ecrite dans ce fichier, trois lignes plus bas, pour les plafonds: « a malformed
   * limit is NOT "no limit" » (audit 2026-07-22). Elle n'avait pas ete appliquee a `exp`. Le champ
   * ABSENT reste licite — une autorisation sans expiration est un choix, tant qu'elle porte une borne de
   * montant — mais un champ PRESENT et illisible est une erreur, pas une permission. */
  if (a.exp != null && a.exp !== '') {
    const expSec = Number(a.exp);
    if (!Number.isFinite(expSec) || expSec <= 0) {
      return { ...base, authorized: false,
        reason: 'exp is present but not a readable unix epoch in seconds (' + JSON.stringify(String(a.exp)).slice(0, 40)
          + ') — a malformed expiry is NOT "never expires"; refusing. Send seconds since 1970, as the JWT `exp` claim does.' };
    }
    if (now > expSec * 1000) return { ...base, authorized: false, reason: 'spend authorization expired — refusing.' };
  }
  ...
  // A PRESENT allow-list is a restriction the OWNER signed — honor it fail-closed (audit 2026-07-22, same
  // discipline as the malformed-limit fix): absent ⇒ no recipient restriction (the caps still bound spend);
  // present-but-empty [] ⇒ NOBODY is allowed (an explicit empty set is not "anyone" — that was the fail-open);
  // present-but-not-an-array ⇒ a malformed restriction, refuse rather than silently ignore it.
  if (a.allowedRecipients != null) {
    if (!Array.isArray(a.allowedRecipients)) return { ...base, authorized: false, reason: 'allowedRecipients is present but not an array — a malformed allow-list is not "no restriction"; refusing.' };
    if (a.allowedRecipients.length === 0) return { ...base, authorized: false, reason: 'allowedRecipients is an empty list — an explicit empty allow-list permits NO recipient (not "anyone"); refusing.' };
