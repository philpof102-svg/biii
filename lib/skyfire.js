...
function authorizeCharge(spendAuth, charge, { spentMicro = 0, now = Date.now(), verified = false } = {}) {
  ...
  if (a.verified !== true) return { ...base, authorized: false, reason: 'spend authorization not verified (verified!==true) — a signed authorization is required; a claim is not one.' };
