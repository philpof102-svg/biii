...
function assessDelivery({ commitment = null, received = undefined, receivedHash = undefined,
  ...
  /* ⚠️ `ordered` testait la PRESENCE la ou il faut la LISIBILITE. Avec des horodatages ISO — le format
   * le plus naturel qu'un appelant puisse envoyer — `Number()` rend NaN: `ordered` valait quand meme
   * true, `NaN >= NaN` etait false, les deux branches etaient sautees, et on tombait sur le `served`
   * final, celui qui affirme le plus. Mesure du 2026-07-28, engagement APRES le paiement:
   *
   *   paidAt='2026-07-28T09:00:00Z', committedAt='2026-07-28T10:00:00Z'
   *     -> verdict `served`, et la phrase: « a commitment recorded at 10:00:00Z, BEFORE the payment at
   *        09:00:00Z ». Elle affirme « avant » en imprimant deux dates qui disent le contraire.
   *
   * Ce n'est pas une reserve manquante: c'est une affirmation fausse, contredite par sa propre preuve
   * citee. Et `Number('0xabc')` vaut 2748, donc un HASH passe pour une date et se fait comparer.
   *
   * On exige donc des nombres FINIS. Un ordre illisible retombe sur la branche honnete (« l'anteriorite
   * n'est pas prouvee »), en disant pourquoi — jamais sur la phrase forte. */
  /* ⚠️ L'HEXADECIMAL EST LEGITIME ICI — les numeros de bloc circulent en `0x…` sur tout l'ecosysteme,
   * et `Number('0x1406f40')` rend correctement 21000000. On ne peut donc pas le rejeter sans casser un
   * appelant juste. Mais un HASH est de la meme forme: `Number('0x' + 'ab'.repeat(32))` vaut 7,76e76 —
   * fini, donc accepte, puis compare comme s'il s'agissait d'une date. Mesure du 2026-07-28: deux hashes
   * produisaient un `commitment_too_late` dont la phrase citait « 0xdef vs 0xabc » comme des instants.
   *
   * La borne est MAX_SAFE_INTEGER, et elle n'est pas arbitraire: un bloc et un epoch en millisecondes
   * tiennent tres largement en dessous, un hash de 32 octets est 60 ordres de grandeur au-dessus, et
   * au-dela la comparaison entiere cesse d'etre fiable de toute facon. */
  const nombreFini = (v) => {
  ...
  const paidNum = nombreFini(paidAt);
  const commitNum = nombreFini(committedAt);
  const ordreFourni = paidAt != null && committedAt != null;
  const ordreIllisible = ordreFourni && (paidNum === null || commitNum === null);
  const ordered = paidNum !== null && commitNum !== null;
  if (ordered && commitNum >= paidNum) {
  ...
  if (!ordered) {
    /* Deux raisons de ne pas pouvoir ordonner, et elles n'appellent pas la meme reparation: rien n'a ete
     * fourni, ou ce qui a ete fourni ne se lit pas comme un nombre. La seconde etait invisible. */
    return { verdict: 'served', matches: true,
      reason: ordreIllisible
        ? 'The received bytes hash to the commitment, so nothing was substituted. Ordering was SUPPLIED but is '
          + 'not readable as a number (paidAt=' + JSON.stringify(String(paidAt)).slice(0, 32)
          + ', committedAt=' + JSON.stringify(String(committedAt)).slice(0, 32) + '), so prior existence is '
          + 'UNPROVEN — this is not "no ordering", it is ordering we could not read.'
        : 'The received bytes hash to the commitment, so nothing was substituted. Ordering was NOT supplied, ' +
          'so prior existence is unproven — pass paidAt and committedAt to get the stronger claim.',
      limits: [QUALITY_LIMIT,
        ordreIllisible
          ? 'Ordering was given but unreadable. Send block numbers or epoch milliseconds — an ISO string or a '
            + 'hash is not compared here, and a hash would silently coerce to a number (Number("0xabc") is 2748).'
